...
class NetBridge(ewrap.EntryWrapper):
    ...
    def bld(cls, pvid, vios_to_backing_adpts, vlan_ids, vswitch):
        ...
        vios_tuple_1 = vios_to_backing_adpts[0]
        nb.seas = [SEA.bld(pvid, vios_tuple_1[0], vios_tuple_1[1], vlan_ids,
                           vswitch, primary=True)]

        # If there is a second VIOS, it should not be marked as primary.
        # There is a maximum of two.  Any more has been indicated will
        # never be supported.
        if len(vios_to_backing_adpts) == 2:
            vios_tuple_2 = vios_to_backing_adpts[1]
            nb.seas.append(SEA.bld(pvid, vios_tuple_2[0], vios_tuple_2[1],
                                   vlan_ids, vswitch, primary=False))
