...
class VNetDevBase(ewrap.EntryWrapper):
    """Base class for a Virtual Network Device.

    In this context, a virtual network device is an object:
    ...
    so that consumers performing generic VNetDev operations might be able to
    ...
    detail: JSON string representing
            {'provider': 'PYPOWERVM_WRAPPER_VNETDEVBASE',
    ...
    """
    EVENT_PROVIDER_ID = 'PYPOWERVM_WRAPPER_VNETDEVBASE'

    @classmethod
    def _push_event(cls, vnd, action, **extra_detail):
        """Push an Event for a virtual network device action.

        :param vnd: VNetDevBase (subclass) object affected by the event.
        ...
        data = u.dice_href(vnd.href, include_scheme_netloc=True,
                           include_query=False, include_fragment=False)
        detail = json.dumps(dict(provider=cls.EVENT_PROVIDER_ID,
                                 action=action, pvid=vnd.pvid, mac=vnd.mac,
                                 **extra_detail))
        evt = event.Event.bld(vnd.adapter, data, detail).create()
        LOG.info(_("Pushed custom event: %s"), str(evt))

    def create(self, **kwargs):
        """Create the vnet dev and push a custom 'create' event.

        :param kwargs: Arguments to EntryWrapper.create.
        :return: Wrapper of the subclass type representing the created object.
        """
        vnd = super(VNetDevBase, self).create(**kwargs)
        self._push_event(vnd, 'create')
        return vnd

    def update(self, **kwargs):
        """Update the vnet dev and push a custom 'update' event.

        :param kwargs: Arguments to EntryWrapper.update.
        :return: Wrapper of the subclass type representing the updated object.
        """
        vnd = super(VNetDevBase, self).update(**kwargs)
        self._push_event(vnd, 'update', orig_pvid=self.pvid, orig_mac=self.mac)
        return vnd

    def delete(self):
        """Delete the vnet dev and push a custom 'delete' event."""
        super(VNetDevBase, self).delete()
        self._push_event(self, 'delete')

    @property
    def mac(self):
        """Returns the Mac Address for the vnet dev.
