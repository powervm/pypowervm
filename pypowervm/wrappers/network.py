...
class NetBridge(ewrap.EntryWrapper):
    ...
    def list_arbitrary_pvids(self, vnet_path=True):
        ...
        :param vnet_path: Should be set by the trait 'vnet_aware'.  Defaults
                          to True.  If True, the Load Groups will be queried.
                          If False, the first SEA's Trunk Adapter will be
                          queried.

                          Only required if modifying the wrapper.
    ...
    def list_vlans(self, pvid=True, arbitrary=False, vnet_path=True):
        ...
        :param vnet_path: Should be set by the trait 'vnet_aware'.  Defaults
                          to True.  If True, the vlans will be listed via
                          the Load Groups.  If False, the first SEA's Trunk
                          Adapters will be queried.
        ...
        if vnet_path:
            ...
            # Loop through the first SEAs trunks
            sea = self.seas[0]
            if pvid:
    ...
    def supports_vlan(self, vlan, vnet_path=True):
        ...
        return vlan in self.list_vlans(vnet_path=vnet_path)
