...
class PhypVMSample(PhypLparSample):
    ...
    def __init__(self, lpar):
        ...
        self.network = None if net is None else Network(net)

        storage = lpar.get('storage')
        self.storage = None if storage is None else Storage(storage)
...
class Network(object):
    ...
    def __init__(self, network):
        ...
        self.veas = [] if veas is None else [VEA(x) for x in veas]

        sriov_ports = network.get('sriovLogicalPorts')
        self.sriov_ports = ([] if sriov_ports is None
                            else [SriovLparPort(x) for x in sriov_ports])
...
class Storage(object):
    ...
    def __init__(self, stor):
        ...
        self.v_stor_adpts = ([] if v_adpts is None
                             else [StorageVAdpt(x) for x in v_adpts])

        v_fcs = stor.get('virtualFiberChannelAdapters')
        self.v_fc_adpts = ([] if v_fcs is None
                           else [VirtualFCAdpt(x) for x in v_fcs])


class StorageVAdpt(object):
