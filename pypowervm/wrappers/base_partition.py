...
class PartitionMemoryConfiguration(ewrap.ElementWrapper):
    ...
    def ppt_ratio(self):
        ...
        The ppt_ratio here is an integer value corresponding to the power of 2
        for the actual ratio. For example if the value is 12, then the ratio is
        1:4096, and a VM with 64 GB of maximum memory would have a PPT of 16 MB
        (64 GB / 4096 = 16 MB).

        NOTE: The default value 0 is not a valid value but represents that the
              VM does not have a PPT ratio.
        """
        return self._get_val_int(_MEM_PPT_RATIO, default=0)
