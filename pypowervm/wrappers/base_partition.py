...
class IOSlot(ewrap.ElementWrapper):
    ...
    The REST XML for ProfileIOSlot contains a nested <AssociatedIOSlot/>
    element, which is represented by the internal AssociatedIOSlot wrapper
    class.  The IOSlot wrapper exposes the AssociatedIOSlot properties directly
    via util.nestedprop:
        - bus_grp_required
        - description
        - phys_loc
    ...
    # == Nested @propertys from AssociatedIOSlot. ==
    bus_grp_required = u.nestedprop(_asio_slot,
                                    AssociatedIOSlot.bus_grp_required)
    description = u.nestedprop(_asio_slot, AssociatedIOSlot.description)
    phys_loc = u.nestedprop(_asio_slot, AssociatedIOSlot.phys_loc)
    pc_adpt_id = u.nestedprop(_asio_slot, AssociatedIOSlot.pc_adpt_id)
    pci_class = u.nestedprop(_asio_slot, AssociatedIOSlot.pci_class)
    pci_dev_id = u.nestedprop(_asio_slot, AssociatedIOSlot.pci_dev_id)
    pci_subsys_dev_id = u.nestedprop(_asio_slot,
                                     AssociatedIOSlot.pci_subsys_dev_id)
    pci_mfg_id = u.nestedprop(_asio_slot, AssociatedIOSlot.pci_mfg_id)
    pci_rev_id = u.nestedprop(_asio_slot, AssociatedIOSlot.pci_rev_id)
    pci_vendor_id = u.nestedprop(_asio_slot, AssociatedIOSlot.pci_vendor_id)
    pci_subsys_vendor_id = u.nestedprop(_asio_slot,
                                        AssociatedIOSlot.pci_subsys_vendor_id)
    drc_index = u.nestedprop(_asio_slot, AssociatedIOSlot.drc_index)
    drc_name = u.nestedprop(_asio_slot, AssociatedIOSlot.drc_name)
    io_adapter = u.nestedprop(_asio_slot, AssociatedIOSlot.io_adapter)
