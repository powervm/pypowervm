...
class VIOS(bp.BasePartition):
    ...
    def update(self, xag=None, timeout=-1):
        ...
        (a lot of I/O devices) or under heavy load (a lot of I/O traffic and/or
        configuration operations in flight) can legitimately take a long time
        to complete - longer than the one-minute timeout that is configured by
        default in pypowervm.adapter.Session.  This override sets the default
        timeout for just VIOS POST operations to a more reasonable five
        minutes, unless the Session timeout is already configured to be longer,
        in which case that value is used.

        Otherwise, the behavior is identical to EntryWrapper.update.
        ...
        """
        if timeout == -1:
            # Override default timeout to five minutes unless the Session is
            # configured for longer already.
            timeout = max(5 * 60, self.adapter.session.timeout)
