...
class VirtualSCSIMapping(ewrap.ElementWrapper):
    ...
    def client_adapter(self):
        ...
        if elem is not None:
            return VirtualSCSIClientAdapter.load(element=elem)
        return None

    @property
    def server_adapter(self):
        """Returns the Virtual I/O Server side VirtualSCSIServerAdapter."""
        return VirtualSCSIServerAdapter.load(
            element=self._element.find(MAP_SERVER_ADAPTER))
    ...
    def backing_storage(self):
        ...
        if elem is not None:
            ...
            if e is not None:
                return storage.VirtualDisk.load(element=e)

            # Check if virtual optical media
            e = elem.find(storage.VOPT_ROOT)
            if e is not None:
                return storage.VirtualOpticalMedia.load(element=e)
...
class VirtualFCMapping(ewrap.ElementWrapper):
    ...
    def client_adapter(self):
        ...
        elem = self._element.find(MAP_CLIENT_ADAPTER)
        if elem is not None:
            return VirtualFCClientAdapter.load(element=elem)
        return None
    ...
    def backing_port(self):
        ...
        if elem is not None:
            return PhysicalFCPort.load(element=elem)
        return None

    @property
    def server_adapter(self):
        """Returns the Virtual I/O Server Virtual FC Adapter."""
        return VirtualFCServerAdapter.load(
            element=self._element.find(MAP_SERVER_ADAPTER))
