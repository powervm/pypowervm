...
class VIOS(ewrap.EntryWrapper):
    ...
    def virtual_scsi_mappings(self):
        return self.entry.element.find(c.VIRT_SCSI_MAPPINGS)

    @property
    def media_repository(self):
        return self.entry.element.find(c.VIRT_MEDIA_REPOSITORY_PATH)
    ...
    def hdisk_reserve_policy(self, disk_uuid):
        ...
        volumes = self.entry.element.findall(c.PVS_PATH)
    ...
    def ip_addresses(self):
        ...
        seas = self.entry.element.findall(c.SHARED_ETHERNET_ADAPTER)
        free_eths = self.entry.element.findall(c.ETHERNET_BACKING_DEVICE)
    ...
    def vfc_mappings(self):
        ...
        def_attrib = self.XAGEnum.VIOS_FC_MAPPING.attrs
    ...
    def vfc_mappings(self, new_mappings):
        self.replace_list(_VIO_VFC_MAPPINGS, new_mappings,
                          attrib=self.XAGEnum.VIOS_SCSI_MAPPING.attrs)

    @property
    def scsi_mappings(self):
        """Returns a WrapperElemList of the VSCSIMapping objects."""
        def_attrib = self.XAGEnum.VIOS_SCSI_MAPPING.attrs
        es = ewrap.WrapperElemList(
    ...
    @property
    def seas(self):
        def_attrib = self.XAGEnum.VIOS_NETWORK.attrs
        es = ewrap.WrapperElemList(
...
class VSCSIMapping(VStorageMapping):
    ...
    def bld(cls, adapter, host_uuid, client_lpar_uuid, stg_name, wcls):
        ...
        map._client_adapter(VClientStorageAdapter.bld_new())
        map._server_adapter(VServerStorageAdapter.bld_new())
    ...
    def backing_storage(self):
        ...
        if elem is not None:
            ...
            e = elem.find(stor.VDisk.schema_type)
            if e is not None:
                return stor.VDisk.wrap(e)
