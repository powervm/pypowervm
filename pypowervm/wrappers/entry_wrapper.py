    ...
    def set_href(self, propname, href):
        ...
        if len(links) == 1:
            ...
            while len(l) > 1:
                ...
                append_point = ElementWrapper.for_propname(next_prop).load(
                    element=new_el)
...
class EntryWrapper(Wrapper):
    ...
    def load(cls, response=None, entry=None, etag=None):
        ...
        :param response: The Response from an adapter.Adapter.read request.
        :param entry: An existing adapter.Entry to wrap.
        :returns: A list of wrappers if a Feed.  A single wrapper if single
                  entry.
        """
        if response is None and entry is None:
            raise ValueError(_("Must specify either the 'response' parameter"
                               "or the 'entry' parameter."))
        if response is not None and entry is not None:
            raise ValueError(_("Cannot specify both the 'response' parameter"
                               "and the 'entry' parameter."))
        ...
        if response is not None:
            if response.entry is not None:
                try:
                    ...
                    etag = None
                wrap = cls.load(entry=response.entry, etag=etag)
                return wrap
            elif response.feed is not None:
                wraps = []
                for entry in response.feed.entries:
                    wraps.append(cls.load(
                        entry=entry, etag=entry.properties.get('etag', None)))
                return wraps
            else:
        ...
        try:
            ...
            class DynamicEntryWrapper(EntryWrapper):
                schema_type = entry.properties.get('title')
...
class ElementWrapper(Wrapper):
    ...
    def load(cls, element=None, **kwargs):
        ...
        try:
            ...
            if element is None:
                raise NotImplementedError(
                    _('Cannot create a dynamic ElementWrapper without a'
                      'schema_type.'))
...
class WrapperElemList(list):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, slice):
            ...
            return [self.child_class.load(element=x, **self.injects)
                    for x in all_elems]

        elem = self.root_elem.findall(self.child_type)[idx]
        return self.child_class.load(element=elem, **self.injects)
    ...
    def __iter__(self):
        elems = self.root_elem.findall(self.child_type)
        for elem in elems:
            yield self.child_class.load(element=elem, **self.injects)
