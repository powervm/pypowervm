...
class TestSearch(unittest.TestCase):
    ...
    def test_no_search_keys(self):
        ...
        with self.assertRaises(ValueError) as arve:
            self.adp.search(clust.Node, foo='bar')
        self.assertEqual(arve.exception.args[0],
                         'Wrapper class Node does not support search.')
    ...
    def test_no_such_search_key(self):
        ...
        self.assertEqual(arve.exception.args[0],
                         "Wrapper class Cluster does not support search key "
                         "'foo'.")
