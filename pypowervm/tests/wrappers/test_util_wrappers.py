...
class TestWrapperElemListFilter(twrap.TestWrapper):
    ...
    def test_filter(self):
        filt = wrappers.WrapperElemListFilter(self.dwrap.vfc_mappings)
        self.assertEqual(filt.results, self.dwrap.vfc_mappings)
        filt.client_lpar_href(self.href)
        expected = [wrap for wrap in self.dwrap.vfc_mappings if
                    wrap.client_lpar_href == self.href]
        self.assertEqual(expected, filt.results)
        self.assertEqual(4, len(filt.results))
        filt.client_adapter(wwpns=self.wwpns)
        expected = [wrap for wrap in expected if
                    wrap.client_adapter.wwpns == self.wwpns]
        self.assertEqual(expected, filt.results)
        self.assertEqual(1, len(filt.results))

    def test_cumulative(self):
        filt = wrappers.WrapperElemListFilter(self.dwrap.vfc_mappings)
        self.assertEqual(58, len(filt.results))
        filt.server_adapter(lpar_slot_num=4)
        self.assertEqual(28, len(filt.results))
        filt.client_lpar_href(self.href)
        self.assertEqual(3, len(filt.results))
    ...
    def test_chaining(self):
        ...
        self.assertEqual(
            ...
            filt.client_lpar_href(
                self.href).client_adapter(wwpns=self.wwpns).results)
        self.assertEqual(1, len(filt.results))
