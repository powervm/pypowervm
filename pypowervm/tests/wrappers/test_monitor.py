...
class TestRawMetrics(twrap.TestWrapper):
    ...
    def test_raw_metrics(self):
        link = ('https://9.1.2.3:12443/rest/api/pcm/ManagedSystem/98498bed-'
                'c78a-3a4f-b90a-4b715418fcb6/RawMetrics/LongTermMonitor/LTM'
                '_8247-22L*1111111_vios_2_20150429T205130+0000.json')

        wrap = self.entries[0]
        self.assertEqual('89b75326-0423-4872-92e6-981fe52138a0', wrap.id)
        self.assertEqual(
            'LTM_8247-22L*1111111_vios_2_20150429T205130+0000.json',
            wrap.title)
        self.assertEqual('2015-04-29T20:51:30.000Z', wrap.updated)
