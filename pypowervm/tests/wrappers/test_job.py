...
class TestJobEntryWrapper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertNotEqual(request, None,
                            "Could not load %s " %
                            JOB_REQUEST_FILE)
        TestJobEntryWrapper._request_wrapper = jwrap.Job.load(
            entry=request.entry)
        # ok wrapper
        ...
        self.assertNotEqual(response, None,
                            "Could not load %s " %
                            JOB_RESPONSE_OK)
        TestJobEntryWrapper._ok_wrapper = jwrap.Job.load(
        ...
        self.assertNotEqual(response, None,
                            "Could not load %s " %
                            JOB_RESPONSE_FAILED)
        TestJobEntryWrapper._failed_wrapper = jwrap.Job.load(
            entry=response.entry)
        # exception wrapper
