...
class TestEntryWrapper(unittest.TestCase):
    ...
    def test_etag(self):
        fake_entry = apt.Entry({'title': 'fake_entry'},
                               apt.Element('fake_entry'))
        etag = '1234'
        ew = ewrap.EntryWrapper.load(entry=fake_entry, etag=etag)
        self.assertEqual(etag, ew.etag)

        ew = ewrap.EntryWrapper.load(entry=fake_entry)
    ...
    def test_load(self):
        ...
        self.assertRaises(KeyError,
                          ewrap.EntryWrapper.load, response=resp)

        # Set an entry...
        entry = apt.Entry({'title': 'entry'}, apt.Element('entry'))
        resp.entry = entry

        # Run
        ew = ewrap.EntryWrapper.load(response=resp)
        ...
        resp2.entry = entry
        # Run
        ew = ewrap.EntryWrapper.load(response=resp2)
        # Validate the etag is None since there were no headers
        self.assertEqual(None, ew.etag)

        # Wipe our entry, add feed.
        resp.entry = None
        e1 = apt.Entry({'etag': '1', 'title': 'entry1'}, None)
        e2 = apt.Entry({'etag': '2', 'title': 'entry2'}, None)
...
class TestElementWrapper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nb1 = ewrap.EntryWrapper.load(entry=self.resp.feed.entries[0])
        self.resp2 = pvmhttp.load_pvm_resp(NET_BRIDGE_FILE).get_response()
        self.nb2 = ewrap.EntryWrapper.load(entry=self.resp2.feed.entries[0])
    ...
    def test_href(self):
        ...
        sea = ewrap.ElementWrapper.load(
            element=self.nb1._find(
                './SharedEthernetAdapters/SharedEthernetAdapter'))
...
class TestWrapperElemList(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wrapper = ewrap.EntryWrapper.load(entry=nb)
    ...
    def test_append(self):
        sea_add = ewrap.ElementWrapper.load(
            element=apt.Element('SharedEthernetAdapter'))
        self.assertEqual(1, len(self.elem_set))
