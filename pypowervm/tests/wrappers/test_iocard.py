...
def test_crud_overrides(test_class, mock_event, mock_del, mock_upd, mock_crt):
    ...
    mock_event.bld.side_effect = validate_event_bld(
        provider=net.VNetDevBase.EVENT_PROVIDER_ID, action='create',
        pvid=test_class.dwrap.pvid, mac=test_class.dwrap.mac)
    # Run the create
    ...
    mock_event.bld.side_effect = validate_event_bld(
        provider=net.VNetDevBase.EVENT_PROVIDER_ID, action='update',
        orig_pvid=test_class.dwrap.pvid, orig_mac=test_class.dwrap.mac,
        pvid=1234, mac='MAC')
    # Run the update
    ...
    # Validate .delete()
    mock_event.bld.side_effect = validate_event_bld(
        provider=net.VNetDevBase.EVENT_PROVIDER_ID, action='delete',
        pvid=test_class.dwrap.pvid, mac=test_class.dwrap.mac)
