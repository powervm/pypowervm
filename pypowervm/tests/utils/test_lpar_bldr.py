...
class TestLPARBuilder(unittest.TestCase):
    ...
    def test_builder(self):
        ...
        attr = dict(name='TheName', env=bp.LPARTypeEnum.AIXLINUX,
                    memory=1024, vcpu=1)
        ...
        # Leave out memory
        attr = dict(name=lpar, env=bp.LPARTypeEnum.AIXLINUX, vcpu=1)
        ...
        attr = dict(name='lpar', memory=3333,
                    env=bp.LPARTypeEnum.AIXLINUX, vcpu=1)
        bldr = lpar_bldr.LPARBuilder(attr, self.stdz_sys1)
        ...
        attr = dict(name='TheName', env=bp.LPARTypeEnum.AIXLINUX,
                    memory=1024, vcpu=1,
                    sharing_mode=bp.SharingModesEnum.CAPPED,
                    srr_capability='true')
        bldr = lpar_bldr.LPARBuilder(attr, self.stdz_sys1)
        new_lpar = bldr.build()
        ...
        attr = dict(name='TheName', env=bp.LPARTypeEnum.AIXLINUX,
                    memory=1024, vcpu=1,
                    sharing_mode=bp.SharingModesEnum.UNCAPPED,
                    uncapped_weight=100)
        ...
        m = bp.DedicatedSharingModesEnum.SHARE_IDLE_PROCS_ALWAYS
        attr = dict(name='TheName', env=bp.LPARTypeEnum.AIXLINUX,
        ...
        attr = dict(name='lpar', memory=2048,
                    env=bp.LPARTypeEnum.AIXLINUX, vcpu=3,
                    max_vcpu=2)
        bldr = lpar_bldr.LPARBuilder(attr, self.stdz_sys1)
        self.assertRaises(ValueError, bldr.build)
