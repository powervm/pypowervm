...
class TestNetworkBridger(unittest.TestCase):
    ...
    def test_ensure_vlan_on_nb_new_vlan(self, mock_adpt, mock_arb_vid,
        ...
        def validate_of_update_nb(*kargs, **kwargs):
            ...
            netbr = pvm_net.NetworkBridge.load(entry=adpt.Entry([], elem))
            self.assertEqual(1,
    ...
    def test_ensure_vlans_on_nb_new_vlan(self, mock_adpt, mock_arb_vid,
        ...
        def validate_of_update_nb(*kargs, **kwargs):
            ...
            elem = kargs[0]
            self.assertIsNotNone(elem)
            net_br = pvm_net.NetworkBridge.load(entry=adpt.Entry({}, elem))
            self.assertEqual(1,
    ...
    def test_find_available_lb(self):
        nb = pvm_net.NetworkBridge.load(response=self.mgr_nbr_resp)
    ...
    def test_find_or_create_vnet(self, mock_adpt):
        ...
        # Load the data
        vnets = pvm_net.VNet.load(response=self.mgr_vnet_resp)
        vsw = pvm_net.VirtualSwitch.load(response=self.mgr_vsw_resp)[0]
    ...
    def test_is_arbitrary_vid(self):
        nbs = pvm_net.NetworkBridge.load(response=self.mgr_nbr_resp)
    ...
    def test_reassign_arbitrary_vid(self, mock_adpt, mock_vsw,
                                    mock_find_vnet):
        nb = pvm_net.NetworkBridge.load(response=self.mgr_nbr_resp)[0]
        host_uuid = 'c5d782c7-44e4-3086-ad15-b16fb039d63b'
    ...
    def test_remove_vlan_from_nb(self, mock_adpt):
        ...
        def validate_update(*kargs, **kwargs):
            ...
            nb = pvm_net.NetworkBridge.load(entry=adpt.Entry({}, kargs[0]))
