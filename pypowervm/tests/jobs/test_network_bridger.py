...
MGR_NET_BR_FILE = 'sea_mgr_network_bridge.txt'
MGR_VNET_FILE = 'sea_mgr_virtual_network.txt'
MGR_VSW_FILE = 'sea_mgr_virtual_switch.txt'
...
class TestNetworkManager(unittest.TestCase):
    ...
    def tearDown(self):
        super(TestNetworkManager, self).tearDown()
    ...
    def test_ensure_vlan_on_nb_new_lg(self, mock_adpt, mock_arb_vid,
        ...
        def validate_of_update_nb(*kargs, **kwargs):
            ...
            self.assertEqual(1,
                             len(net_br.load_grps[0].virtual_network_uri_list))
            self.assertEqual(2,
                             len(net_br.load_grps[2].virtual_network_uri_list))
    ...
    def test_find_available_lb(self):
        nb = pvm_net.NetworkBridge.load_from_response(self.mgr_nbr_resp)
    ...
    def test_reassign_arbitrary_vid(self, mock_adpt, mock_vsw,
                                    mock_find_vnet):
        nb = pvm_net.NetworkBridge.load_from_response(self.mgr_nbr_resp)[0]
        host_uuid = 'c5d782c7-44e4-3086-ad15-b16fb039d63b'
