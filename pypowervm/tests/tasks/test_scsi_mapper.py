...
class TestSCSIMapper(testtools.TestCase):
    ...
    def test_index_mappings(self):
        ...
        self.assertEqual({
            'by-lpar-id', 'by-lpar-uuid', 'by-storage-udid', 'by-target-udid'},
            set(idx.keys()))
        ...
        # Only five mappings have target devs, and all are different
        self.assertEqual(5, len(idx['by-target-udid'].keys()))
        for sudid in idx['by-target-udid']:
            self.assertEqual(1, len(idx['by-target-udid'][sudid]))
