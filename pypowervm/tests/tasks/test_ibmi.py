...
CNA_FEED = 'cna_feed.txt'


class TestIBMiWithPVM(testtools.TestCase):
    """Unit Tests for IBMi changes."""

    def setUp(self):
        super(TestIBMiWithPVM, self).setUp()
        ...
        self.canlist = [self.cna_feed[0], self.cna_feed[1]]
    ...
    def test_update_ibmi_settings(self, mock_viosw, mock_cnaw):
        ...
        mock_cnaw.return_value = self.canlist
...
class TestIBMiWithHMC(testtools.TestCase):
    """Unit Tests for IBMi changes."""

    def setUp(self):
        super(TestIBMiWithHMC, self).setUp()
        self.apt = self.useFixture(pvm_fx.AdapterFx(
            traits=pvm_fx.RemoteHMCTraits)).adpt
        self.vio_feed = pvm_vios.VIOS.wrap(
            tju.load_file(VIOS_FEED, self.apt))
        self.vioslist = [self.vio_feed[0], self.vio_feed[1]]
        self.cna_feed = pvm_net.CNA.wrap(
