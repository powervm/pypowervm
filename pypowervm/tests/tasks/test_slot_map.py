...
class TestSlotMapStoreLegacy(testtools.TestCase):
    ...
    def test_max_slot(self):
        """Test setting/getting the max_slot."""
        smt = self.smt_impl('foo')
        # Starts off unset
        self.assertIsNone(smt.max_slot)
        # Can assign initially
        smt.register_max_slot(123)
        self.assertEqual(123, smt.max_slot)
        # Can overwrite
        smt.register_max_slot(234)
        self.assertEqual(234, smt.max_slot)
        ...
        # max_slot still set
        self.assertEqual(234, smt.max_slot)
        # Topology not polluted by max_slot
        self.assertEqual({3: {'VFC': {'fab1': None, 'fab10': None,
                                      'fab11': None, 'fab12': None,
                                      'fab13': None, 'fab14': None,
                                      'fab15': None, 'fab16': None,
...
class TestRebuildSlotMapLegacy(testtools.TestCase):
    ...
    def test_max_slot(self):
        """Ensure max_slot returns the set value, or 10 + highest slot."""
        # With max_slot unset...
        smt = self.smt_impl('foo')
        smt._slot_topo = {3: {'CNA': {'5E372CFD9E6D': 'ETHERNET0'}},
                          4: {'CNA': {'2A2E57A4DE9C': 'ETHERNET0'}},
                          6: {'CNA': {'3AEAC528A7E3': 'MGMTSWITCH'}}}
        rsm = slot_map.RebuildSlotMap(smt, [self.vio1, self.vio2], None, {})
        # ...max_slot is 10 + highest slot in the topo)
        self.assertEqual(16, rsm.get_max_slot())

        # With max_slot set...
        smt.register_max_slot(123)
        rsm = slot_map.RebuildSlotMap(smt, [self.vio1, self.vio2], None, {})
        # ...max_slot returns the exact value
        self.assertEqual(123, rsm.get_max_slot())
