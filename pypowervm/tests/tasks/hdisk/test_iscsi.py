...
class TestIscsi(testtools.TestCase):
    ...
    def test_discover_volume(self, mock_job_res, mock_job_p, mock_run_job,
                             mock_job_w):
        ...
        mock_iqn = 'fake_iqn'
        mock_uuid = 'uuid'
        args = ['ManagedSystem', mock_uuid]
        kwargs = {'suffix_type': 'do', 'suffix_parm': ('ISCSIDiscovery')}
        mock_job_res.return_value = {'DEV_OUTPUT': ['fake_iqn devName']}
        device_name = iscsi.discover_volume(
            self.adpt, mock_uuid, mock_host_ip, mock_user, mock_pass, mock_iqn)
