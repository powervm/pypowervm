...
class TestGetOrUploadImageLU(twrap.TestWrapper):
    ...
    def upload_new_lu_mock(self, crt_raise=None, upl_raise=None):
        """Generate a mock side effect for upload_new_lu2.

        :param crt_raise: Exception to be raised by the (simulated) crt_lu of
                          the img_lu.  If None (the default), the img_lu is
                          created (added to the SSP).
        :param upl_raise: Exception to be raised by the (simulated) upload_lu
                          part of upload_new_lu2.  If None (the default), the
                          upload "succeeds" and we return (ssp, img_lu, None).
                          Ignored if crt_raise is not None.
        :return: A callable suitable for assigning to
    ...
    def test_upload_raises_non_dup_on_crt(self):
        ...
        self.mock_crt_lu.side_effect = self.crt_mkr_lu_mock(
            conflicting_mkr_lu=self.confl_mkr_lu_lose)
        self.mock_upload_new_lu.side_effect = self.upload_new_lu_mock(
            crt_raise=IOError('crt_lu raises non-DuplicateLUNameError within '
                              'upload_new_lu2'))
    ...
    def test_upload_raises_dup_on_crt(self):
        ...
        self.mock_crt_lu.side_effect = self.crt_mkr_lu_mock(
            conflicting_mkr_lu=self.confl_mkr_lu_lose)
        self.mock_upload_new_lu.side_effect = self.upload_new_lu_mock(
            crt_raise=ex.DuplicateLUNameError(lu_name=self.img_lu.name,
    ...
    def test_upload_raises_on_upload(self):
        """I win; upload_new_lu2 raises after crt_lu (during upload_lu)."""
        self.mock_crt_lu.side_effect = self.crt_mkr_lu_mock(
            conflicting_mkr_lu=self.confl_mkr_lu_lose)
        self.mock_upload_new_lu.side_effect = self.upload_new_lu_mock(
            upl_raise=IOError('upload_lu raises within upload_new_lu2 after '
                              'crt_lu'))
