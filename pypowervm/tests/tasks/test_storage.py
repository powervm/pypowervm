...
class TestScrub2(testtools.TestCase):
    ...
    def test_lu_vopt_vdisk(self, mock_rm_lu, mock_rm_vopt, mock_rm_vd):
        def verify_rm_stg_call(exp_list):
            ...
            return _rm_stg
        mock_rm_lu.side_effect = verify_rm_stg_call(
            [self.vio_feed[0].scsi_mappings[4].backing_storage])
        mock_rm_vopt.side_effect = verify_rm_stg_call(
            [self.vio_feed[0].scsi_mappings[5].backing_storage])
        mock_rm_vd.side_effect = verify_rm_stg_call(
            [self.vio_feed[0].scsi_mappings[8].backing_storage])
