...
class TestVDisk(testtools.TestCase):
    ...
    def test_rescan_vstor(self, mock_adpt_read, mock_run_job):
        vios = mock.Mock(adapter=None, uuid='vios_uuid')
        vopt = mock.Mock(adapter=None, udid='stor_udid')
        ...
        self.assertRaises(exc.AdapterNotFound, ts.rescan_vstor, vios, vopt)
        self.assertEqual(0, self.adpt.read.call_count)
        self.assertEqual(0, mock_run_job.call_count)

        # Add valid adapter
        vios.adapter = self.adpt
        ts.rescan_vstor(vios, vopt)
