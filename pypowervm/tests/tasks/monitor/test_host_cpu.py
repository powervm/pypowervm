...
from pypowervm.tasks.monitor import util as pvm_util
import pypowervm.tests.test_fixtures as pvm_fx

LOG = logging.getLogger(__name__)


class TestHostCPUStats(testtools.TestCase):
    ...
    def setUp(self):
        ...
        pvm_util.ensure_ltm_monitors = mock.MagicMock()
        pvm_util.MetricCache._refresh_if_needed = mock.MagicMock()
    ...
    @mock.patch('pypowervm.tasks.monitor.host_cpu.HostCPUStats.'
                '_get_fw_cycles_delta')
    @mock.patch('pypowervm.tasks.monitor.host_cpu.HostCPUStats._get_cpu_freq')
    @mock.patch('pypowervm.tasks.monitor.host_cpu.HostCPUStats.'
                '_get_total_cycles_delta')
    @mock.patch('pypowervm.tasks.monitor.host_cpu.HostCPUStats.'
                '_gather_user_cycles_delta')
    def test_update_internal_metric(self, mock_user_cycles, mock_total_cycles,
                                    mock_cpu_freq, mock_fw_cycles):

        host_stats = host_cpu.HostCPUStats(self.adpt, 'host_uuid')
    ...
    def test_update_internal_metric_bad_total(
            self, mock_cpu_freq, mock_user_cycles, mock_tot_cycles,
            mock_fw_cycles):
    ...
    def test_get_cpu_freq(self, mock_cmd):
        host_stats = host_cpu.HostCPUStats(self.adpt, 'host_uuid')
