...
from pypowervm.tasks import fc_mapper
...
class TestFCMapper(unittest.TestCase):
    ...
    def test_build_wwpn_pair(self):
        ...
        i = 0
        while i < 100:
            wwpn_pair = fc_mapper.build_wwpn_pair(None, None)
    ...
    def test_find_vio_for_wwpn(self):
        ...
        # Basic test
        vio_resp, p_resp = fc_mapper.find_vio_for_wwpn(
    ...
    def test_derive_npiv_map(self):
        ...
        resp = fc_mapper.derive_npiv_map(vios_wraps, p_wwpns, v_port_wwpns)
    ...
    def test_derive_npiv_map_failure(self):
        ...
        # Run the derivation now
        self.assertRaises(e.UnableToFindFCPortMap, fc_mapper.derive_npiv_map,
    ...
    def test_find_map_port(self):
        ...
        p1 = fc_mapper._find_map_port(vios_w.pfc_ports, [])
        self.assertIsNotNone(p1)

        # Lets add a mapping where P1 is used.  Should not get that result
        # back.
        p2 = fc_mapper._find_map_port(vios_w.pfc_ports, [(p1.wwpn, '')])
        self.assertIsNotNone(p2)
        self.assertNotEqual(p1, p2)

        # Now add a third and fourth port.  Same assertions.
        p3 = fc_mapper._find_map_port(vios_w.pfc_ports, [(p1.wwpn, ''),
                                                         (p2.wwpn, '')])
        self.assertIsNotNone(p3)
        self.assertNotIn(p3, [p1, p2])

        p4 = fc_mapper._find_map_port(vios_w.pfc_ports, [(p1.wwpn, ''),
                                                         (p2.wwpn, ''),
                                                         (p3.wwpn, '')])
        ...
        p_temp = fc_mapper._find_map_port(vios_w.pfc_ports, port_use)
        self.assertIsNotNone(p_temp)
        self.assertNotIn(p_temp, [p1, p2, p4])

    def test_fuse_vfc_ports(self):
        self.assertEqual(['A B'], fc_mapper._fuse_vfc_ports(['a', 'b']))
        self.assertEqual(['AA BB'], fc_mapper._fuse_vfc_ports(['a:a', 'b:b']))
        self.assertEqual(['A B', 'C D'],
                         fc_mapper._fuse_vfc_ports(['a', 'b', 'c', 'd']))
...
class TestPortMappings(twrap.TestWrapper):
    ...
    def test_add_port_mapping_multi_vios(self):
        ...
        # Get the mappings
        fabric_A_maps = fc_mapper.derive_npiv_map(vios_wraps, fabric_A_wwpns,
                                                  v_fabric_A_wwpns)
        vios_wraps.reverse()
        fabric_B_maps = fc_mapper.derive_npiv_map(vios_wraps, fabric_B_wwpns,
                                                  v_fabric_B_wwpns)
        full_map = fabric_A_maps + fabric_B_maps

        # Now call the add action
        fc_mapper.add_npiv_port_mappings(self.adpt, 'host_uuid', 'vm_uuid',
                                         full_map)
    ...
    def test_remove_port_mapping_multi_vios(self):
        ...
        # Now call the add action
        fc_mapper.remove_npiv_port_mappings(self.adpt, 'host_uuid', maps)
    ...
    def ensure_does_not_have_wwpns(self, vios_w, wwpns):
        for vfc_map in vios_w.vfc_mappings:
            ...
            for c_wwpn in vfc_map.client_adapter.wwpns:
                if c_wwpn in wwpns:
                    self.fail("WWPN %s in client adapter" % fc_mapper)
    ...
    def test_find_maps(self):
        ...
        matches = fc_mapper.find_maps(vwrap.vfc_mappings, 10)
        ...
        # Bogus LPAR ID
        self.assertEqual([], fc_mapper.find_maps(vwrap.vfc_mappings, 1000))
