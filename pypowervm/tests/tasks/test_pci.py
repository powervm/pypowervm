...
class TestPCI(testtools.TestCase):
    ...
    def _set_up_slot_assignment(self):
        sass = pci._SlotAssignment(self.sfeed[0].asio_config.io_slots[0], 'id')
        # GETs not called yet
        self.mock_lpar.assert_not_called()
        self.mock_vios.assert_not_called()
        return sass

    def test_slot_assignemnt_lazy_load_exception(self):
        sass = self._set_up_slot_assignment()
        self.mock_lpar.return_value = None
        self.mock_vios.return_value = None
        self.assertRaises(exc.NoPartitionForSlotAssignment,
                          lambda: sass.part_w)
        self.mock_lpar.assert_called_once_with(self.adap, uuid='id')
        self.mock_vios.assert_called_once_with(self.adap, uuid='id')

    def test_slot_assignment_lazy_load_lpar(self):
        sass = self._set_up_slot_assignment()
        # Finds the LPAR - VIOS GET not called
        self.mock_lpar.return_value = 'lpar wrapper'
        self.assertEqual('lpar wrapper', sass.part_w)
        self.mock_lpar.assert_called_once_with(self.adap, uuid='id')
        self.mock_vios.assert_not_called()
        # And it caches
        self.assertEqual('lpar wrapper', sass.part_w)
    ...
    def test_slot_assignment_lazy_load_vios(self):
        sass = self._set_up_slot_assignment()
        # Finds the VIOS - LPAR get is called first
        self.mock_lpar.return_value = None
        self.mock_vios.return_value = 'vios wrapper'
        self.assertEqual('vios wrapper', sass.part_w)
        self.mock_lpar.assert_called_once_with(self.adap, uuid='id')
        self.mock_vios.assert_called_once_with(self.adap, uuid='id')
        # And it caches
        self.assertEqual('vios wrapper', sass.part_w)
