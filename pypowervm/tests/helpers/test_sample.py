...
class TestSampleHelper(unittest.TestCase):
    ...
    def test_sample_helper(self, mock_sleep, mock_sess):

        helpers = smpl_hlp.sample_retry_helper
        mock_sess.request.side_effect = pvmex.Error('yo', response=fake_resp1)
        ...
        try:
            ...
            self.assertEqual(mock_sess.request.call_count, 6)
