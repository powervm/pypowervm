...
class TransactionSubtask(object):
    ...
    argument.  It must indicate whether changes were made to the wrapper: this
    is the trigger used by Transaction to determine whether to POST the changes
    back to the REST server via update().
    ...
    def __init__(self, *save_args, **save_kwargs):
        ...
        :param save_args: Positional arguments to be passed to the _execute
                          method - *after* the wrapper - when it is invoked
                          under a Transaction.
        :param save_kwargs: Keyword arguments to be passed to the _execute
    ...
    def _execute(self, *args, **kwargs):
        ...
        The _execute method has two responsibilities:
        ...
        Example:
        def _execute(thingy_wrapper, primary_widget, widget_list, option=True):
            update_needed = False
        ...
        :param args: Positional arguments accepted by the _execute method.  The
                     first argument will always be the EntryWrapper.  Overrides
                     may define their signatures using explicit parameter
                     names.
        :param kwargs: Keyword arguments accepted by the _execute method.
                       Overrides may use explicit parameter names.
        :return: The return value may be either:
                 o A single value which evaluates to True or False, but is NOT
                   an EntryWrapper.  Any True value indicates that the wrapper
                   was modified and should be POSTed back to the REST server
                   via update().  Any False value (including None, [], {}, etc)
                   indicates that this TransactionSubtask did not modify the
                   wrapper.  (Note that it may still be POSTed if modified by
                   other TransactionSubtasks in the same Transaction.)
                 o A two-member tuple whose first value is the (possibly
                   modified) wrapper, and whose second value can be evaluated
                   as a boolean.  The second value indicates whether to POST
                   the wrapper, as specified above.
        """
    ...
    def execute(self, wrapper):
        """Invoke this TransactionSubtask with a wrapper and saved arguments.

        :param wrapper: EntryWrapper instance with which to invoke this
                        TransactionSubtask.  This will be the first argument to
                        the TransactionSubTask's _execute method; subsequent
                        arguments will be those passed to the initializer.
        :return: The (possibly modified) wrapper.
        """
        ret = self._execute(wrapper, *self.save_args, **self.save_kwargs)
        retex = ex.TransactionSubtaskReturnInvalid(
            clazz=self.__class__.__name__, ret=ret)
        if isinstance(ret, tuple):
            # ret is a tuple.  It must have exactly two items.
            if len(ret) != 2:
                raise retex
            new_wrapper, changes = ret
            # The first item  must be the wrapper.
            if not isinstance(new_wrapper, wrapper.__class__):
                raise retex
        else:
            # ret is a single value.  Wrapper is unchanged.
            new_wrapper, changes = wrapper, ret
            # ret must not be (any variant of) the wrapper.
            if isinstance(changes, wrapper.__class__):
                raise retex
        self._update_needed = bool(changes)
        return new_wrapper
...
class Transaction(tf_task.BaseTask):
    ...
    class ModifyGizmos(TransactionSubtask):
        def _execute(self, wrapper, gizmo_list):
            update_needed = False
            if gizmo_list:
                wrapper.gizmos.append(gizmo_list)
                update_needed = True
            return wrapper, update_needed
    ...
    ...
    class _FunctorTransactionSubtask(TransactionSubtask):
        ...
        def _execute(self, wrapper, *_args, **_kwargs):
    ...
    def add_functor_subtask(self, func, *args, **kwargs):
        ...
        :param func: A callable to be the core of the TransactionSubtask.  The
                     ...
                     TransactionSubtask._execute - see that method's docstring
                     for details.
