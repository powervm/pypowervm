...
class FeedManager(object):
    ...
    def flow(self):
        ...
        if self._flow is None:
            ...
            # Wrapping this FeedManager's execution in a Task allows us to
            # defer GETting the feed and replicating the Transactions until the
            # flow is actually run.  In the best case, where the consumer has
            # never invoked any of the 'greedy' methods, this can occur
            # immediately before the Transactions begin to execute, thus
            # minimizing the probability of needing retries.
            self._flow.add(tf_task.FunctorTask(self.execute))
        return self._flow
    ...
    def execute(self):
        ...
        pflow.add(*self.transactions.values())
