...
class FeedManager(tf_task.BaseTask):
    ...
    def __init__(self, name, feed_or_getter, max_workers=10):
        ...
        """
        super(FeedManager, self).__init__(name)
    ...
    def transactions(self):
        ...
        if not self._tx_by_uuid:
            ...
            # As long as the consumer uses FeedManager.add_[functor_]subtask
            # and doesn't ask for .transactions, we keep only one copy of the
            # subtask list.  Once the consumer "breaks the seal" and requests
            # individual transactions per wrapper, we need to (GET the feed -
            # this is triggered by .feed - and) create them based on this
            # common subtask list.
            # This is only done once.  Thereafter, .add_[functor_]subtask will
            # add separately to each Transaction.
