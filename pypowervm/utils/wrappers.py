...
class WrapperElemListFilter(object):
    ...
    - Initialize the filter with the WrapperElemList
        ...
        filt = WrapperElemListFilter(vios_wrapper.vfc_mappings)
    ...
    - Results of the filtering are available at any point via the .results
      attribute (which had better never be the name of an ElementWrapper
      @property).  The results will always be a list, a subset of the original
      WrapperElemList given to the constructor.  It may be identical to the
      original; or empty; or anywhere in between.

        for wrapper in filt.results:
            # Do whatever
    ...
    - Filter methods return the filter, so they can be chained.
            ...
            results = WrapperElemListFilter(wrapper.things)
                      .flat_prop1(val)
                      .flat_prop2(val)
