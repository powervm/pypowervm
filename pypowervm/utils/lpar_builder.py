...
class DefaultStandardize(Standardize):
    ...
    def _can_secure_boot_for_lpar(self, lpar_type):
    ...
    def general(self):
        ...
        if self._can_secure_boot_for_lpar(bld_attr[ENV]):
            self._set_val(bld_attr, SECURE_BOOT, self.secure_boot,
                          convert_func=int)
        LOG.info(f"DBG: self.vsn is in general {self.virtual_serial_number}")
        self._set_val(bld_attr, VIRTUAL_SERIAL_NUMBER, self.virtual_serial_number,
                convert_func=str)
