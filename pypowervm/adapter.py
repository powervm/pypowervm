...
class Session(object):
    ...
    def get_event_listener(self):
        if not self.has_event_listener:
            LOG.info(_LI("setting up event listener for %s") % self.host)
...
class _EventListener(EventListener):
    def __init__(self, session, timeout=-1, interval=15):
        ...
        try:
            ...
            raise pvmex.Error(_('Failed to initialize event feed listener: %s')
                              % str(e))
    ...
    def getevents(self):
        ...
        try:
            ...
            LOG.exception(_('Error while getting PowerVM events.  Returning '
