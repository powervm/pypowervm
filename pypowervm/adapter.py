...
class EventListener(_EventListener):
    """Legacy EventListener class."""
    def __init__(self, session, timeout=-1, interval=15):
        super(EventListener, self).__init__(session, timeout=timeout,
                                            interval=interval)
        # Since _EventListner uses a weakref to the Session, we need to
        # anchor it hear for the legacy case of creating an EventListener
    ...
    def shutdown(self):
        super(EventListener, self).shutdown()
        self._unanchor()
