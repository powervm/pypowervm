...
def build_itls(i_wwpns, t_wwpns, lun):
    ...
    """
    resp = []
    for i_wwpn in i_wwpns:
        for t_wwpn in t_wwpns:
            resp.append(ITL(i_wwpn, t_wwpn, lun))
    return resp
...
def discover_hdisk(adapter, vios_uuid, itls, vendor=LUA_TYPE_IBM):
    ...
    :param vios: The Virtual I/O Server wrapper.
    ...
    resp = adapter.read(pvm_vios.VIO_ROOT, root_id=vios_uuid,
                        suffix_type=c.SUFFIX_TYPE_DO, suffix_parm=LUA_RECOVERY)
    job_wrapper = pvm_job.Job(resp.entry)
