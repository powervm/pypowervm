...
def ensure_vlans_on_nb(adapter, host_uuid, nb_uuid, vlan_ids):
    ...
    nb_wraps = pvm_net.NetworkBridge.load(response=nb_feed)
    ...
    for vlan_id in new_vlans:
        ...
        if ld_grp is None:
            ...
            ld_grp = pvm_net.LoadGroup.load(
                element=pvm_net.crt_load_group(arb_vid, vnet_uris))
...
def _find_vswitch(adapter, host_uuid, vswitch_id):
    ...
    vswitches = pvm_net.VirtualSwitch.load(response=resp_feed)
...
def _find_or_create_vnet(adapter, host_uuid, vnets, vlan, vswitch,
    ...
    resp = adapter.create(vnet_elem, pvm_ms.MS_ROOT, host_uuid,
                          pvm_net.VNET_ROOT)
    return pvm_net.VNet.load(response=resp)
...
def _reassign_arbitrary_vid(adapter, host_uuid, old_vid, new_vid, impacted_nb):
    ...
    new_lg_w = pvm_net.LoadGroup.load(element=new_lg)
    ...
    impacted_nb = pvm_net.NetworkBridge.load(response=nb_resp)
...
def _find_vnet_uri_from_lg(adapter, lg, vlan):
    ...
    for vnet_uri in lg.virtual_network_uri_list:
        ...
        vnet_net = pvm_net.VNet.load(response=vnet_resp)
