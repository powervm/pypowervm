...
def crt_cna(adapter, host_uuid, lpar_uuid, pvid, vswitch='ETHERNET0',
    ...
    :param mac_addr: The optional mac address.  If not specified, one will be
                     auto generated.
    :param addl_vlans: Optional list of (up to 18) additional VLANs.
    """
    ...
    vswitch_href = None
    vswitch_resp = adapter.read('ManagedSystem', host_uuid, 'VirtualSwitch')
    ...
    if vswitch_href is None:
        raise exc.Error("Unable to find the Virtual Switch %s on the system."
                        % vswitch)

    cna = network.crt_cna(pvid, vswitch_href, slot_num, mac_addr,
                          addl_tagged_vlans)
    resp = adapter.create(cna, 'ManagedSystem', host_uuid,
