...
def upload_new_vdisk(adapter, v_uuid,  vol_grp_uuid, d_stream,
    ...
    vol_grp = stor.VolumeGroup.load(entry=vol_grp_data.entry)
    ...
    new_vdisk = stor.VirtualDisk.load(
        element=stor.crt_virtual_disk_obj(d_name, gb_size))
    ...
    resp = adapter.update(vol_grp._entry.element, vol_grp_data.headers['etag'],
                          ...
                          xag=None)
    vol_grp = stor.VolumeGroup.load(entry=resp.entry)
...
def _create_file(adapter, f_name, f_type, v_uuid, sha_chksum=None, f_size=None,
    ...
    return vf.File.load(response=resp)
