...
def _upload_stream(adapter, vio_file, d_stream):
    ...
    try:
        if vio_file.enum_type == vf.FileType.DISK_IMAGE_COORDINATED:
            ...
            fw = None
            try:
                with open(vio_file.asset_file, 'a') as out_stream:
...
class _StreamWriter(threading.Thread):
    ...
    This is a thread that will pipe a stream to an out stream.  Will stop
    ...
    def run(self):
        try:
            shutil.copyfileobj(self.instream, self.outstream)
