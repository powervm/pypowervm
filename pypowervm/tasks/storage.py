...
def _rest_api_pipe(file_writer):
    ...
    try:
        ...
        with futures.ThreadPoolExecutor(1) as th_pool:
            ...
            fifo_reader = util.eintr_retry(open, file_path, 'r')
        ...
        if fifo_reader:
            util.eintr_retry(fifo_reader.close)
        if file_path:
            os.remove(file_path)
        if temp_dir:
            os.rmdir(temp_dir)
...
def _copy_func(out_file, io_handle):
    LOG.info(_("Starting to read from stream for image upload to %s."),
             out_file)
    i = 0
    with util.eintr_retry(open, out_file, 'a+b', 0) as out_str:
        while True:
            chunk = util.eintr_retry(io_handle.read, 65536)
            if not chunk:
                LOG.debug("Reached EOF at chunk %d", i)
                break
            util.eintr_retry(out_str.write, chunk)
