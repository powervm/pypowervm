...
class UploadType(object):
    ...
    """
    The data stream (either a file handle or stream) to upload.  Must have the
    'read' method that returns a chunk of bytes.
    """
    IO_STREAM = 'stream'

    """
    A method function (as described above) that will be invoked to stream the
    data into the virtual disk. Only one parameter is passed in, and that is
    the path to the file to stream the data into.

    This upload_xx method will not return until an EOF is sent to the file path
    passed in to the delegate function.  It is the responsibility of the
...
def upload_new_vdisk(adapter, v_uuid, vol_grp_uuid, io_handle, d_name, f_size,
    ...
    :param vol_grp_uuid: The volume group that will host the Virtual Disk's
                         UUID.
    :param io_handle: The I/O handle (as defined by the upload_method)
    ...
    :param upload_type: (Optional, Default: IO_STREAM) Defines the way in
                        ...
                        UploadType enumeration for valid upload methods.
...
def _upload_stream(vio_file, io_handle, upload_type):
    ...
    try:
        ...
        if upload_type == UploadType.FUNC:
            _upload_stream_delegate(vio_file, io_handle)
            ...
            _upload_stream_coordinated(vio_file, io_handle)
        else:
