...
def _upload_stream(adapter, vio_file, d_stream):
    ...
    try:
        if vio_file.enum_type == vf.FileType.DISK_IMAGE_COORDINATED:
            ...
            # Used for the threading.  As this is a blocking API.
            def upload():
                adapter.upload_file(vio_file.element, EmptyReader())

            def stream():
                with open(vio_file.asset_file, 'ab', 0) as out_stream:
                    shutil.copyfileobj(d_stream, out_stream)

            executor = futures.ThreadPoolExecutor(max_workers=2)
            upload_f = executor.submit(upload)
            stream_f = executor.submit(stream)
