...
def scrub_storage_for_lpar(adapter, lpar_id, vioses=None):
    ...
    :return vwrap_list: List of all inspected VIOS wrappers in their final
                        (possibly updated) form.
    """
    if not vioses:
    ...
    for vwrap in vioses:
        ...
        if not (vfc_removals or vscsi_removals):
            vwrap_rets.append(vwrap)
            continue

        # Update the VIOS to remove the matching mappings
        XXX YOU ARE ALSO HERE.  PARALLEL UPDATES.
        vwrap = vwrap.update()
        vwrap_rets.append(vwrap)

        # Figure out which storage elements need to be removed.
        # o VFC mappings don't have storage we can get to, so ignore those.
        # o Some VSCSI mappings may not have backing storage.
        stg_els_to_remove = [rmap.backing_storage for rmap in vscsi_removals
