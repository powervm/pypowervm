...
def add_lpar_storage_scrub_tasks(lpar_ids, ftsk, lpars_exist=False):
    ...
    def remove_chain(vwrap, lids, stg_type):
        """_remove_lpar_maps with an additional check for extant LPARs."""
        if not lpars_exist:
            lids = set(lids)
            ...
            preserve_lpars = lids & ex_lpar_ids
            if preserve_lpars:
                LOG.warn(_("Skipping scrub of %(stg_type)s mappings from VIOS "
                         ...
                         dict(stg_type=stg_type, vios_name=vwrap.name,
                              lpar_ids=list(preserve_lpars)))
                lids -= preserve_lpars
        return _remove_lpar_maps(vwrap, lids, stg_type)

    ftsk.add_functor_subtask(remove_chain, lpar_ids, 'VSCSI',
                             provides='vscsi_removals_' + tag)
    ftsk.add_functor_subtask(remove_chain, lpar_ids, 'VFC')
