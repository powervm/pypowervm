...
def _upload_stream(adapter, vio_file, d_stream):
    ...
    try:
        ...
        if vio_file.enum_type == vf.FileType.DISK_IMAGE_COORDINATED:
            ...
            # Invoke the upload, but the content is empty.
        ...
        try:
            ...
            # If the file writer was initialized, also tell it to stop, but
            # hide the exception.  The error was because of something wrong
            # with the file...
            if fw:
                fw.stop(raise_error=False)
            return vio_file

        # If the file writer was initialized, also tell it to stop
        if fw:
            fw.stop(raise_error=True)
    return None
...
class _FileWriter(threading.Thread):
    ...
    def run(self):
        ...
        # The file won't exist on the system until the PUT (which invokes this)
        # starts up.  So we have to wait until the file shows up on the system
        # until we start writing it.
        #
        # We limit the loop to 120 checks, a half second apart so that we don't
        # get stuck.
        count = 0
        while count < 120:
        ...
        # At the end of 60...even if it doesn't exist we can still put the file
        # down.  It just won't be a stream, which is not as efficient.
        try:
