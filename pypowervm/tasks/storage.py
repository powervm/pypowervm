...
def crt_lu(tier_or_ssp, name, size, thin=None, typ=None):
    ...
    tier = default_tier_for_ssp(tier_or_ssp) if use_ssp else tier_or_ssp
...
def _rm_lus(all_lus, lus_to_rm, del_unused_images=True):
    ...
    for lu in lus_to_rm:
        ...
        if removed:
            ...
            LOG.info(_("LU %(lu_name)s was not found - it may have been "
                       "deleted out of band.  (UDID: %(lu_udid)s)"),
                     {'lu_name': lu.name, 'lu_udid': lu.udid})
    ...
    for backing_image in backing_images:
        ...
        if backing_image is not None:
            msg_args = dict(lu_name=backing_image.name,
                            lu_udid=backing_image.udid)
            # Only remove backing images that are not in use.
            if _image_lu_in_use(all_lus, backing_image):
                LOG.debug("Not removing Image LU %(lu_name)s because it is "
                          "still in use.  (UDID: %(lu_udid)s)" %
                          msg_args)
            else:
                removed = _rm_dev_by_udid(backing_image, all_lus)
                if removed:
                    LOG.info(_("Removing Image LU %(lu_name)s because it is "
                               "no longer in use.  (UDID: %(lu_udid)s)"),
                             msg_args)
                    changes.append(removed)
                else:
                    # This would be wildly unexpected
                    LOG.warning(_("Backing LU %(lu_name)s was not found.  "
                                  "(UDID: %(lu_udid)s)"), msg_args)
    return changes
...
def rm_tier_storage(lus_to_rm, tier=None, lufeed=None, del_unused_images=True):
    ...
    for dlu in _rm_lus(lufeed, lus_to_rm, del_unused_images=del_unused_images):
        ...
        try:
            ...
            LOG.warning(he)
            LOG.warning(_("LU %(lu_name)s may have been deleted out of band.  "
                          "(UDID: %(lu_udid)s)"), msg_args)
