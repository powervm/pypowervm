...
def upload_new_vdisk(adapter, v_uuid, vol_grp_uuid, io_handle, d_name, f_size,
    ...
    # Virtual Disk
    vio_file = _create_file(adapter, d_name, v_uuid, f_size=f_size,
...
def upload_lu(v_uuid, lu, io_handle, f_size, sha_chksum=None,
    ...
    vio_file = _create_file(lu.adapter, lu.name, v_uuid, f_size=f_size,
...
def _copy_func(out_file, io_handle):
    try:
        LOG.info(_("Starting to read from stream for image upload to %s."),
                 out_file)
        i = 0
        with subprocess._eintr_retry_call(open, out_file, 'a+b', 0) as out_str:
            while True:
                chunk = io_handle.read(65536)
                if not chunk:
                    LOG.debug("Reached EOF at chunk %d", i)
                    break
                subprocess._eintr_retry_call(out_str.write, chunk)
        ...
        LOG.error(_("Encountered an error while uploading to file %s to the "
                    "server."), out_file)
...
def _rest_api_pipe(file_writer):
    ...
    try:
        ...
        with futures.ThreadPoolExecutor(1) as th_pool:
            ...
            fifo_reader = subprocess._eintr_retry_call(open, file_path, 'r')
        ...
        if fifo_reader:
            subprocess._eintr_retry_call(fifo_reader.close)
        if file_path:
            os.remove(file_path)
        if temp_dir:
            os.rmdir(temp_dir)
...
def _create_file(adapter, f_name, v_uuid, sha_chksum=None, f_size=None,
                 tdev_udid=None):
    ...
    # file copy.  Otherwise legacy REST API-based upload.
    f_type = (vf.FileType.DISK_IMAGE_COORDINATED if adapter.traits.local_api
