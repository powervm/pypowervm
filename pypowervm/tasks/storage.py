...
def rm_lu(adapter, ssp, lu=None, udid=None, name=None, update=True):
    ...
    def _op():
        ...
        lus.remove(lu_to_rm)
        if update:
            return ssp.update(adapter), lu_to_rm
        else:
            return ssp, lu_to_rm
    ...
    # Since the locks don't get absorbed via the call stack, we have to
    # separate out the lock flows.  No update, no need to lock.  If we have
    # the update, we should obtain the lock.
    if update:
        return _op_with_lock()
    else:
        return _op()
