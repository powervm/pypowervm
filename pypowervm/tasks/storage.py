...
def _upload_stream_local(vio_file, io_handle, upload_type):
    ...
    with futures.ThreadPoolExecutor(max_workers=2) as th:
        ...
        # The real upload thread, wrapped to ensure the file/pipe gets closed.
        copy_f = th.submit(_wrap_stream_func, func, vio_file.asset_file, *argv)
        ...
        for fut in futures.as_completed([upload_f, copy_f]):
            if fut is upload_f:
                util.future_log_if_exception(
                    upload_f, _("Dummy API upload encountered an error "
                                "uploading file %s"), vio_file.asset_file)
            else:
                util.future_log_if_exception(
                    copy_f, _("Coordinated copying function encountered an "
                              "error uploading file %s."), vio_file.asset_file)
...
def _wrap_stream_func(func, asset_file, *argv):
    try:
        func(asset_file, *argv)
    finally:
