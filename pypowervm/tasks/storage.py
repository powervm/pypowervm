...
def find_lus_in_ssp(ssp, lu_name, lu_type=None, whole_name=False):
    """Find LUs within an SSP by name and possibly type.

    :param ssp: SSP wrapper to search for the LUs.
    :param lu_name: The name of the LUs to find.
    :param lu_type: (Optional) The type of the LU to find.  If unspecified,
                    only the name is considered.
    :param whole_name: (Optional) If True, the lu_name must match exactly.  If
                       False (the default), match any name containing lu_name
                       as a substring.
    :return: A list of zero or more matching LU wrappers.
    """
    matches = []
    for lu in ssp.logical_units:
...
def find_lu_in_ssp(ssp, lu_name, lu_type=None):
    """Find a specified LU within an SSP by name and possibly type.

    :param ssp: SSP wrapper to search for the LU.
    :param lu_name: The name of the LU to find.  Exact match.
    :param lu_type: (Optional) The type of the LU to find.  If unspecified,
                    only the name is considered.
    :return: The wrapper of the first matching LU, or None if not found.
    """
    matches = find_lus_in_ssp(ssp, lu_name, lu_type=lu_type,
                              whole_name=True)
