...
def add_lpar_storage_scrub_tasks(lpar_id, ftsk):
    ...
    def remove_storage(wrapper_task_rets):
        ...
        for vuuid, rets in wrapper_task_rets.items():
            ...
            vg_wraps = stor.VG.wrap(vwrap.adapter.read(
        ...
        if lus_to_rm:
            ...
            # loop.
            ssplist = stor.SSP.wrap(lus_to_rm[0].adapter.read(
                stor.SSP.schema_type))
    ...
    ftsk.add_functor_subtask(rm_vscsi_maps, provides='vscsi_removals')
    ftsk.add_functor_subtask(rm_vfc_maps)
    ftsk.add_post_execute(tf_tsk.FunctorTask(remove_storage))
    # Caller is responsible for executing ftsk.
