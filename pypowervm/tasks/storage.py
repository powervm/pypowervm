...
def _rm_lus(ssp_wrap, lus, del_unused_images=True):
    ...
    for lu in lus:
        ...
        try:
            ssp_lus.remove(lu)
            LOG.info("Removing LU %(lu_name)s from SSP %(ssp_name)s"
                     % msg_args)
            changes.append(lu)
        except ValueError:
    ...
    for backing_image in backing_images:
        ...
        if backing_image is not None:
            ...
            if _image_lu_in_use(ssp_wrap, backing_image):
                ...
                LOG.info("Removing Image LU %(lu_name)s from SSP %(ssp_name)s "
                         "because it is no longer in use." % msg_args)
        ...
        for vuuid, rets in wrapper_task_rets.items():
            ...
            for stg in stg_els_to_remove:
                if isinstance(stg, stor.LU):
                    ...
                    luftsk.add_functor_subtask(_rm_lus, [stg])
            ...
            if vdisks_to_rm:
                vgftsk.add_functor_subtask(_rm_vdisks, vdisks_to_rm)
            if vopts_to_rm:
                vgftsk.add_functor_subtask(_rm_vopts, vopts_to_rm)
