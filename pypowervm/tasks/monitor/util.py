...
def latest_stats(adapter, host_uuid, include_vio=True, second_latest=False):
    ...
    latest_phyp, sec_latest_phyp = _get_latest_metrics(ltm_metrics, 'phyp')
...
def get_lpar_metrics(ltm_metrics, adapter, second_latest=False):
    ...
    """
    latest_lpar, sec_latest_lpar = _get_latest_metrics(ltm_metrics, 'lpar')
    for metric in ltm_metrics:
...
def _get_latest_metrics(metrics, metric_type):
    def compare(met1, met2):
        return cmp(met1.updated_datetime, met2.updated_datetime)
    filtered = [met for met in metrics if met.category == metric_type]
    metrics = sorted(filtered, cmp=compare, reverse=True)
