...
class LparMetricCache(object):
    ...
    def _parse_current_feed(self):
        ...
        :return: Two elements.
                 - The date of the metrics gathered.  If no metrics, is the
                   current date time (so metrics are constantly re-queried when
                   the system is unable to gather metrics (but rather at the
                   recommended time interval).
                 - The current date when the metrics were retrieved.  Does
                   not utilize the date within the metric (as there may be
                   clock skew).
        """
