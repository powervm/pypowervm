...
def check_iscsi_discovery(vios_uuid, status):
    """Processes the iscsiadm return codes
     ...
     :param status: iscsiadm return code for the successful discovery,
                    status should be ISCSI_SUCCESS or ISCSI_ERR_SESS_EXISTS.
...
def check_iscsi_logout(vios_uuid, status):
    """Check if logout is successful
     ...
     :param vios_uuid: The uuid of the VIOS.
     :param status: iscsiadm return code for the successful logout,
                    status should be ISCSI_SUCCESS or ISCSI_ERR_NO_OBJS_FOUND.
     :return: True on success otherwise raise exception.
    """
...
def discover_iscsi(adapter, vios_uuid, iscsi_data):
    ...
    :param iscsi_data: ISCSIInputData object with required input parameters
                       for discovery.
