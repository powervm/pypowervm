...
def _process_iscsi_result(status, job=None):
    ...
    if status == ISCSIStatus.ISCSI_COMMAND_NOT_FOUND:
        ...
        return False
    if job == _JOB_NAME and status not in [ISCSIStatus.ISCSI_SUCCESS,
                                           ISCSIStatus.ISCSI_ERR_SESS_EXISTS]:
        raise pexc.ISCSIDiscoveryFailed(vios_uuid=vios_uuid, status=status)

    if (job == _ISCSI_LOGOUT and status not in
...
def discover_iscsi(adapter, host_ip, user, password, iqn, vios_uuid,
                   target_lun, transport_type=None):
    ...
    job_parms.append(job_wrapper.create_job_parameter('targetLUN', target_lun))
    ...
    try:
        ...
        if not good_iscsi_discovery(results.get('RETURN_CODE')):
            return None, None
...
def discover_iscsi_initiator(adapter, vios_uuid):
    ...
    try:
        ...
        if good_iscsi_discovery(results.get('RETURN_CODE')):
...
def remove_iscsi(adapter, targetIQN, vios_uuid):
    ...
    try:
        ...
        # RETURN_CODE: for iscsiadm status
        return good_iscsi_discovery(results.get('RETURN_CODE'))
