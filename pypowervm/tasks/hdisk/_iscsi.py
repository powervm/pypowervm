...
def discover_iscsi(adapter, host_ip, user, password, iqn, vios_uuid,
    ...
    :param transport_type: Transport type of the volume to be connected.
    ...
    _add_parameter(job_parms, 'transportType', transport_type)
...
def remove_iscsi(adapter, targetIQN, vios_uuid, transport=None, lun=None,
    ...
    :param vios_uuid: The uuid of the VIOS (VIOS must be a Novalink VIOS type).
    :param transport: Transport type of the volume to be connected.
    ...
    job_wrapper = job.Job.wrap(resp)
    job_parms = []
    _add_parameter(job_parms, 'transport', transport)
