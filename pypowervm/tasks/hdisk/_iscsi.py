...
def _log_iscsi_status(status):
    ...
    LOG.info(_("ISCSI Discovery command return code =%s"), status)
    ...
    if status == ISCSIStatus.ISCSI_ERR_ODM_QUERY:
        LOG.warning(_("ISCSI discovery found the stale entries in "
                      "the ODM database. Scrub the stale entries "
                      "and retry."))
    if status == ISCSIStatus.ISCSI_ERR_SESS_NOT_FOUND:
        LOG.warning(_("ISCSI session could not be found "))
    if status == ISCSIStatus.ISCSI_ERR_NO_OBJS_FOUND:
        LOG.warning(_("No records/targets/sessions/portals "
                      "found to execute operation on"))
    if status == ISCSIStatus.ISCSI_ERR_SESS_EXISTS:
        LOG.warning(_("ISCSI session already exists and logged in"))
...
def _process_iscsi_result(result, iqn, host_ip):
    ...
    if status == ISCSIStatus.ISCSI_COMMAND_NOT_FOUND:
        _log_iscsi_status(status, host_ip)
        return None, None, None
