...
def _find_free_vlan(adapter, host_uuid, src_vios_uuid, vswitch_w):
    """Finds a free VLAN on the vswitch specified."""
    # TODO(thorst) talk to clbush, kriskend, seroyer and efried
    # clbush - how do we search from trunk adapter on the novalink
    # efried - q's on search syntax...
    # kriskend/seroyer - extend to support multiple vioses
    trunk_list = pvm_net.CNA.search(
        adapter, parent_type=lpar.LPAR, parent_uuid=src_vios_uuid,
        vswitch_uri=vswitch_w.href)

    used_pvids = [x.pvid for x in trunk_list]
    for t in trunk_list:
        used_pvids.extend(t.tagged_vlans)

    for x in range(1, 4094):
        if x not in used_pvids:
            return x
...
def crt_p2p_cna(adapter, host_uuid, lpar_uuid, src_vios_uuid, vswitch,
    ...
    :param host_uuid: The host UUID that the CNA will be put on.
    :param lpar_uuid: The lpar UUID to update.
    :param src_vios_uuid: The UUID of the LPAR (ex. a VIOS) that will host the
                          corresponding trunk adapters.
    :param pvid: The primary VLAN ID.
    ...
    # Find the free VLAN
    vlan = _find_free_vlan(adapter, host_uuid, src_vios_uuid, vswitch_w)
    ...
    # TODO(thorst) Talk to clbush to see if this is right...
    # TODO(kriskend/seroyer) How do we want to handle this parent type?
    trunk_adpt = pvm_net.CNA.bld(
        adapter, vlan, vswitch_w.related_href, trunk_pri=1)
    trunk_adpt = trunk_adpt.create(parent_type=lpar.LPAR,
                                   parent_uuid=src_vios_uuid)
    return client_adpt, trunk_adpt
