...
def migrate_lpar(
    ...
    adapter and the second is what the target PVID should be set to for that
    adapter.  The remaining is a list of additional VLANs that could be
    specified for adapters that have additional VLANs.  Note the backslash and
    comma to delimit the additional VLANs.  There should be a single backslash
    in the string, or two if using non-raw python strings.  If not specified,
    then no additional VLANs will be on the target adapter
