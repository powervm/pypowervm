...
def migrate_lpar(lpar, tgt_mgd_sys, validate=False,
                 tgt_mgmt_plfm=None, tgt_mgmt_usr=None,
    ...
    :param validate: Indication of whether to just validate the migration
        or actually perform it.
    :param tgt_mgmt_plfm: The ip of the PowerVM management platform managing
        the target host.
    ...
    op = _SUFFIX_PARM_MIGRATE_VALIDATE if validate else _SUFFIX_PARM_MIGRATE
    ...
    job_parms = [job_wrapper.create_job_parameter(TGT_MGD_SYS,
                                                  str(tgt_mgd_sys))]
    for kw, val in [(TGT_RMT_HMC, tgt_mgmt_plfm),
