...
def generate_lpm_mappings(lpar, vios_wraps):
    ...
    has to be sent in the specific format. This method is used to generates
    those formatted mapping strings of virtual_fc_mappings and
    virtual_scsi_mappings for a given lpar. Iterates through the available
    VIOS's find the matching mapping for the lpar and return the comma
    separated list of VFC/VSCSI Mappings which can be fed to migrate_lpar
    command input.
    ...
    :param vios_wraps: List of pypowervm vios wrappers called with XAGs for
                       storage and fibre-channel. If the VIOS wrapper does not
