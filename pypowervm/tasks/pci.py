...
class _SlotAssignment(object):
    ...
    they're getting.
    """
    def __init__(self, ioslot_w, part=None):
        """Create a _SlotAssignment.

        :param ioslot_w: IOSlot (from System, LPAR, or VIOS) whose assignment
                         information is being recorded.
        :param part: LPAR or VIOS wrapper or UUID of the partition to which the
                     ioslot_w is assigned; or None if the slot is unassigned.
        """
        self._ioslot_w = ioslot_w
        self._adap = ioslot_w.adapter
        _is_wrapper = isinstance(part, bp.BasePartition)
        self._part_w = part if _is_wrapper else None
        self.part_uuid = part.uuid if _is_wrapper else part
    ...
    def __new__(cls, adap, sys_w=None):
        ...
        for part_w in lfeed + vfeed:
            for slot in part_w.io_config.io_slots:
                sa_list.append(_SlotAssignment(slot, part=part_w))
        ...
        # Construct the tuple
        return super(SlotInfo, cls).__new__(cls, sa_list)
