...
def power_off_vfcs(part, fail_if_invalid=False):
    ...
    """
    return _power_vfcs(part, pexc.VFCPowerOffFailed,
                       storage.WWPNStatus.LOGGED_OUT,
                       fail_if_invalid=fail_if_invalid)


def power_on_vfcs(part, wwpn, fail_if_invalid=False):
    ...
    :param part: The LPAR wrapper.
    :param wwpn: The WWPN on the vFC to log in.  Only one should be logged in.
    :param fail_if_invalid: (Optional, Default: False) If something is wrong
                            and this is set to True, will raise the
                            failure_error.  Examples: LPAR is powered on.
    :return: True if the action completed successfully.  Returns False if the
             ...
             system and therefore could not perform the action.
    """
    return _power_vfcs(part, pexc.VFCPowerOnFailed,
                       storage.WWPNStatus.LOGGED_IN,
                       fail_if_invalid=fail_if_invalid, wwpn=wwpn)
...
def _power_vfcs(part, failure_error, port_state, fail_if_invalid=False,
    ...
    :param fail_if_invalid: (Optional, Default: False) If something is wrong
                            and this is set to True, will raise the
                            failure_error.  Examples: LPAR is powered on.
    :param wwpn: (Optional, Default: None) If not set, will perform the action
                 against all of the vFC WWPNs on the client adapter.  If set
                 will only perform the action against the single adapter.
    :return: True if the action completed successfully.  Returns False if the
             fail_if_invalid is set to False, but something was wrong with the
             system and therefore could not perform the action.
    """
    wwpn = u.sanitize_wwpn_for_api(wwpn) if wwpn else None
    ...
    for client_adpt in client_adpts:
        ...
        for n_port in n_ports:
            ...
            # then skip.
            if wwpn and u.sanitize_wwpn_for_api(n_port.wwpn) != wwpn:
                continue
