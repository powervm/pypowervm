...
class PowerOp(object):
    ...
    Use the pwron or pwroff @classmethod to invoke the appropriate Job.  Jobs
    ...
    def __init__(self, part, suff=_SUFF_PWROFF, oper=None, immed=False):
        ...
        Do not call this directly; use the pwron or pwroff @classmethod.
    ...
    @classmethod
    def pwron(cls, part, add_parms, timeout=CONF.pypowervm_job_request_timeout,
              synchronous=True):
        ...
        try:
            ...
            if (any(err_prefix in emsg
                ...
                LOG.debug("Partition %s already powered on.", part.name)
                return
            LOG.exception(error)
            raise pexc.VMPowerOnFailure(lpar_nm=part.name, reason=emsg)

    @classmethod
    def pwroff(cls, part, oper=None, hard=False, restart=False, add_parms=None,
               timeout=CONF.pypowervm_job_request_timeout, synchronous=True):
        """Set up a PowerOff Job.
        ...
        try:
            ...
            if (any(err_prefix in emsg
                    ...
                    not restart):
                LOG.debug("Partition %s already powered off.", part.name)
                return
            LOG.exception(error)
...
def power_on(part, host_uuid, add_parms=None, synchronous=True):
    ...
    """
    PowerOp.pwron(part, add_parms, synchronous=synchronous)
...
def _pwroff_soft_ibmi_flow(part, force_immediate, restart, timeout, add_parms):
    ...
    if not immed:
        # ==> OS normal
        try:
            PowerOp.pwroff(part, oper=PowerOp.OP_PWROFF_OS, hard=False,
                           restart=restart, add_parms=add_parms,
                           timeout=timeout)
            return True
        except pexc.VMPowerOffFailure:
            LOG.debug("IBMi OS normal shutdown failed; trying OS "
                      "immediate shutdown.  Partition: %s", part.name)
            # Fall through to OS immediate

    # ==> OS immediate
    try:
        PowerOp.pwroff(part, oper=PowerOp.OP_PWROFF_OS, hard=True,
                       restart=restart, add_parms=add_parms,
                       timeout=timeout)
        return True
    except pexc.VMPowerOffFailure:
        LOG.debug("IBMi OS immediate shutdown failed; trying VSP normal "
                  "shutdown.  Partition: %s", part.name)
        # Fall through to VSP normal

    # ==> VSP normal
    try:
        PowerOp.pwroff(part, oper=PowerOp.OP_PWROFF_VSP, hard=False,
                       restart=restart, add_parms=add_parms,
                       timeout=timeout)
        return True
    except pexc.VMPowerOffFailure:
        LOG.debug("IBMi VSP normal shutdown failed.  Partition: %s",
                  part.name)
        # If NO_RETRY, that's it.
        if force_immediate == Force.NO_RETRY:
            LOG.debug("NO_RETRY specfied; skipping VSP hard shutdown.")
            raise

    return False


def _pwroff_soft_non_ibmi_flow(part, force_immediate, restart, timeout,
                               add_parms):
    ...
    try:
        ...
        LOG.debug("Non-IBMi OS immediate shutdown timed out; skipping normal "
                  "VSP shutdown.  Partition: %s", part.name)
        # If NO_RETRY, that's it.
        if force_immediate == Force.NO_RETRY:
            LOG.debug("NO_RETRY specfied; skipping VSP hard shutdown.")
            raise
        ...
        LOG.debug("Non-IBMi OS immediate shutdown failed; trying VSP "
    ...
    try:
        ...
        LOG.debug("Non-IBMi VSP normal shutdown failed.  Partition: "
                  "%s", part.name)
        # If NO_RETRY, that's it.
        if force_immediate == Force.NO_RETRY:
            LOG.debug("NO_RETRY specfied; skipping VSP hard shutdown.")
...
def power_off(part, host_uuid, force_immediate=Force.ON_FAILURE, restart=False,
    ...
    if force_immediate == Force.TRUE:
        PowerOp.pwroff(part, oper=PowerOp.OP_PWROFF_VSP, hard=True,
                       restart=restart, add_parms=add_parms, timeout=timeout)
        return
    ...
    if part.env == bp.LPARType.OS400:
        ...
        if _pwroff_soft_non_ibmi_flow(part, force_immediate, restart, timeout,
                                      add_parms):
            return
        # Fall through to VSP hard
    ...
    # ==> VSP hard
    LOG.debug("VSP hard shutdown with default timeout.  Partition: %s",
              part.name)
    PowerOp.pwroff(part, oper=PowerOp.OP_PWROFF_VSP, hard=True,
                   restart=restart, add_parms=add_parms)
