...
class PowerOp(object):
    ...
    """
    _OP_PWRON = 'PowerOn||'
    # See the stop classmethod for the behavior of OP_PWROFF_*.
    ...
    def start(cls, part, add_parms=None,
        ...
        :param part: Partition (LPAR or VIOS) wrapper indicating the partition
                     to power on.
        :param add_parms: A dict of parameters to the Job.  BootMode,
                          KeylockPos, and RemoveOptical provide commonly-used
                          keys/values.
...
def _pwroff_soft_ibmi_flow(part, timeout, add_parms):
    ...
    if add_parms.get('immediate') != 'true':
        # ==> OS normal
        try:
...
def _power_off_single(part, force_immediate, add_parms, timeout):
    """No-retry single power-off operation.

    :param part force_immediate add_parms timeout: See power_off.
                                    force_immediate is either TRUE or NO_RETRY.
                                    add_parms must not be None.
...
def power_off(part, host_uuid, force_immediate=Force.ON_FAILURE, restart=False,
    ...
    if force_immediate != Force.ON_FAILURE:
        return _power_off_single(part, force_immediate, add_parms, timeout)
