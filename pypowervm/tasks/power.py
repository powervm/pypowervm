...
def _pwroff_soft_ibmi_flow(part, restart, timeout, add_parms):
    ...
    :param part restart timeout add_parms: See power_off.
...
def _pwroff_soft_standard_flow(part, restart, timeout, add_parms):
...
def power_off(part, host_uuid, force_immediate=Force.ON_FAILURE, restart=False,
    ...
    if force_immediate == Force.NO_RETRY:
        ...
        hard = part.env != bp.LPARType.OS400
        PowerOp.stop(part, hard=hard, restart=restart, add_parms=add_parms,
                     timeout=timeout)
        return
    ...
    if part.env == bp.LPARType.OS400:
        ...
        if _pwroff_soft_ibmi_flow(part, restart, timeout, add_parms):
            return
            # Fall through to VSP hard
    else:
        # The non-IBMi progression.
        if _pwroff_soft_standard_flow(part, restart, timeout, add_parms):
            return
            # Fall through to VSP hard
