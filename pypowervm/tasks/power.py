...
class PowerOp(object):
    ...
    def start(cls, part, add_parms=None,
        ...
        try:
            pwrop.run(add_parms or {}, timeout, synchronous=synchronous)
    ...
    def stop(cls, part, oper=None, hard=False, restart=False, add_parms=None,
        ...
        add_parms = dict(add_parms) if add_parms else {}
        ...
        try:
            pwrop.run(add_parms, timeout, synchronous=synchronous)
        except pexc.JobRequestTimedOut as error:
            LOG.exception(error)
    ...
    def run(self, add_parms, timeout, synchronous=True):
...
def _pwroff_soft_ibmi_flow(part, force_immediate, restart, timeout, add_parms):
    ...
    if not immed:
        ...
        try:
            ...
            LOG.warning(_("IBMi OS normal shutdown failed.  Partition: %s"),
                        part.name)
            if force_immediate == Force.NO_RETRY:
                raise
            LOG.warning(_("Trying OS immediate shutdown.  Partition: %s"),
                        part.name)
    ...
    try:
        ...
        LOG.warning(_("IBMi OS immediate shutdown failed.  Partition: %s"),
                    part.name)
        if force_immediate == Force.NO_RETRY:
            raise
        LOG.warning(_("Trying VSP normal shutdown.  Partition: %s"), part.name)
    ...
    try:
        ...
        if force_immediate == Force.NO_RETRY:
            raise
        LOG.warning(_("Trying VSP hard shutdown.  Partition %s"), part.name)
...
def _pwroff_soft_standard_flow(part, force_immediate, restart, timeout,
    ...
    try:
        ...
        LOG.warning(_("Non-IBMi OS immediate shutdown timed out.  Partition: "
                      "%s"), part.name)
        if force_immediate == Force.NO_RETRY:
            raise
        LOG.warning(_("Trying VSP hard shutdown.  Partition: %s"), part.name)
        ...
        LOG.warning(_("Non-IBMi OS immediate shutdown failed.  Partition: %s"),
                    part.name)
        if force_immediate == Force.NO_RETRY:
            raise
