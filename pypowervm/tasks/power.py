...
def power_on(part, host_uuid, add_parms=None, synchronous=True):
    ...
    """
    if isinstance(add_parms, popts.PowerOnOpts):
        opts = add_parms
    else:
        if add_parms is not None:
            import warnings
            warnings.warn(_("Specifying add_parms as a dict is deprecated. "
                            "Please specify a power_opts.PowerOnOpts instance "
                            "instead."), DeprecationWarning)
        opts = popts.PowerOnOpts(legacy_add_parms=add_parms)

    PowerOp.start(part, opts=opts, synchronous=synchronous)
...
def power_off(part, host_uuid, force_immediate=Force.ON_FAILURE, restart=False,
    ...
    :raise VMPowerOffTimeout: If the operation timed out (possibly after
                              retrying).
    """
    if isinstance(add_parms, popts.PowerOffOpts):
        opts = add_parms
    else:
        if add_parms is not None:
            import warnings
            warnings.warn(_("Specifying add_parms as a dict is deprecated. "
                            "Please specify a power_opts.PowerOffOpts "
                            "instance instead."), DeprecationWarning)
        opts = popts.PowerOffOpts(legacy_add_parms=add_parms)
