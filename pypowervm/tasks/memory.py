...
def calculate_memory_overhead_on_host(adapter, host_uuid,
    ...
    :parm adapter: pypowervm adapter
    :parm host_uuid: the UUID of the host
    :parm reserve_mem_data: dictionary with values for job params
    :parm lmb_size: logical memory block size
    :parm max_mem: maximum memory MB of an instance during deploy
    :parm default: default value to use for required memory overhead
                   value if there was an error with the job
    :returns tuple of ints PHYP memory required. (Required memory -
             desired memory) and current free memory on the host
    """
    ...
    max_mem = reserved_mem_data.get('max_mem',
                                    max_mem)
    ...
    if lmb_size is not None:
        max_mem = int(math.ceil((max_mem or 1) / float(lmb_size)) *
                      int(lmb_size))
    num_virt_eth_adapter = reserved_mem_data.get('num_virt_eth_adapters')
    ...
    job_parms = [job_wrapper.create_job_parameter(
        'LogicalPartitionEnvironment', 'AIX/Linux')]
    job_parms.append(job_wrapper.create_job_parameter(
        'DesiredMemory', str(desired_mem)))
    job_parms.append(job_wrapper.create_job_parameter(
        'MaximumMemory', str(max_mem)))
    # If number of virtual ethernet adapters is provided, use it,
    # otherwise it will be defaulted to 2.
    if num_virt_eth_adapter is not None:
        job_parms.append(job_wrapper.create_job_parameter(
            'NumberOfVirtualEthernetAdapter',
            str(num_virt_eth_adapter)))
    job_parms.append(job_wrapper.create_job_parameter(
        'NumberOfVirtualSCSIAdapter', '1'))
