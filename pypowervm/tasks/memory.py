...
def calculate_memory_overhead_on_host(adapter, host_uuid,
    ...
    :parm reserve_mem_data: optional dictionary with values for job params
        {'desired_mem': int,
         'max_mem': int,
    ...
    :parm lmb_size: logical memory block size (optional)
    :parm default: default value to use for required memory overhead
                   value if there was an error with the job (optional)
    :returns overhead: reserved host memory
    :returns avail_mem: available host memory
    """
