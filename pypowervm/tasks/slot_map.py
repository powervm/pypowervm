...
class SlotMapBase(object):
    ...
    def register_vfc_mapping(self, vfcmap, fabric):
        ...
        :param vfcmap: VFCMapping ElementWrapper representing the mapping to
                       be incorporated.
        :param fabric: The fabric name associated with the mapping.
        """
        self._reg_slot(IO_CLASS.VFC, fabric, vfcmap.server_adapter.lpar_id)

    def register_vscsi_mapping(self, vscsimap):
        """Incorporate the slot topology associated with a VSCSI mapping.

        Different storage types are treated differently.  See the topology
        @property.
        ...
        cslot = vscsimap.server_adapter.lpar_id
        ...
        if isinstance(bstor, stor.VDisk):
            ...
            # Save the UDID in case it's a local rebuild.
    ...
    def topology(self):
        ...
        - io_key: The unique identifier of the mapped I/O element.  This
                  differs by IO_CLASS type as follows:
            IO_CLASS    stg_key
            CNA         CNA.mac
            VDISK       VDisk.udid
            PV          PV.udid
