...
class BuildSlotMap(object):
    ...
    def get_vfc_slot(self, vios_w, fabric):
        ...
        :return: Integer client slot number on which to map the given fabric on
                 the given VIOS.
        """
        npiv_map = self._build_map.get(IOCLASS.VFC, {})
...
class RebuildSlotMap(BuildSlotMap):
    ...
    def __init__(self, slot_store, vios_wraps, pv_vscsi_vol_to_vio,
        ...
        :param npiv_fabric_map: Mapping of vFC fabrics to lists of physical
                                WWPNs.  Of the following format:
                                { 'fabric': ['aa', 'bb'] }
        """
    ...
    def _pv_vscsi_build_out(self, vol_to_vio):
        ...
        for slot in slots_order:
            ...
            for udid in self._slot_store.topology[slot][IOCLASS.PV]:
                ...
                self._put_stg_slot(IOCLASS.PV, vios_uuid_for_slot, udid, slot)
    ...
    def _npiv_build_out(self, fabric_pwwpn_map):
        """Builds the build map for the NPIV fabrics."""
        seen_fabrics = set()
        for fabric, p_wwpns in six.iteritems(fabric_pwwpn_map):
            # {'phys_wwpns': [p_wwpn, ...], 'slots': [slot, ...] }
            fabric_slot_map = {'phys_wwpns': p_wwpns}
            ...
            for slot, iomap in six.iteritems(self._slot_store.topology):
                if fabric not in iomap.get(IOCLASS.VFC, {}):
                    continue
                if 'slots' not in fabric_slot_map:
                    fabric_slot_map['slots'] = []
                fabric_slot_map['slots'].append(slot)
                seen_fabrics.add(fabric)

            self._npiv_fabric_build_out(fabric_slot_map)
        ...
        self._all_fabrics_seen(seen_fabrics)

    def _all_fabrics_seen(self, seen_fabrics):
        """Make sure all the topology's fabrics are accounted for.

        :param seen_fabrics: Set of fabrics handled by _npiv_build_out.
    ...
    def _npiv_fabric_build_out(self, fabric):
        ...
        # Copy so we don't modify the _slot_map.
        necessary_slots = copy.deepcopy(slots)

        # See if we can get enough viable VIOSes for the slots.  One client
        # slot per VIOS.
        for vios in self.vios_wraps:
            ...
            for phys_ports in phys_wwpns:
                if phys_ports in vios.get_pfc_wwpns():
                    ...
                    self._put_stg_slot(IOCLASS.VFC, vios.uuid, fabric, slot)
                    break
    ...
    def _put_vea_slot(self, mac, slot):
        if IOCLASS.CNA not in self._build_map:
            self._build_map[IOCLASS.CNA] = {}
        self._build_map[IOCLASS.CNA][mac] = slot
    ...
    def _put_stg_slot(self, stg_class, vios_uuid, stg_key, slot):
        ...
        self._build_map[stg_class][vios_uuid][stg_key] = slot
