...
class RebuildSlotMap(object):
    def __init__(self, slot_topo, vios_wraps, pv_vscsi_vol_to_vio,
                 npiv_fabric_map):
        ...
        :param slot_topo: Slot topology produced from slot_map.topology, where
                          slot_map is a SlotMapBase (or subclass) instance.
    ...
    def _pv_vscsi_build_out(self, vol_to_vio):
        ...
        for slot in slots_order:
            if not self.topology[slot].get(IOCLASS.PV):
                continue
            ...
            for udid in self.topology[slot][IOCLASS.PV]:
    ...
    def _vea_build_out(self):
        ...
        for slot, io_dict in six.iteritems(self.topology):
