...
class RebuildSlotMap(BuildSlotMap):
    ...
    def _vscsi_build_out(self, vol_to_vio):
        ...
        for slot in slots_order:
            ...
            slot_info = slot_topo.get(IOCLASS.PV, {}).items()
            slot_info.extend(slot_topo.get(IOCLASS.LU, {}).items())

            for udid, lua in slot_info:
            ...
            for udid, lua in slot_info:
                if slot_topo.get(IOCLASS.PV, {}).get(udid):
                    stg_class = IOCLASS.PV
                else:
                    stg_class = IOCLASS.LU
