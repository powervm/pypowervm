...
class BuildSlotMap(object):
    ...
    This base implementation is for creating or modifying an existing instance.
    It generally returns a client slot that allows the deploy implementation to
    ...
    def get_pv_vscsi_slot(self, vios_w, udid):
        ...
        # mapping).
        pv_vscsi_map = self._build_map.get('pv_vscsi', {})
    ...
    def get_vea_slot(self, mac):
        ...
        return self._build_map.get('vea', {}).get(
    ...
    def get_mgmt_vea_slot(self):
        ...
        """
        mgmt_vea = self._build_map.get('mgmt_vea', {})
    ...
    def get_vfc_slot(self, vios_w, fabric):
        ...
        :return: Integer client slot number on which to map the given fabric on
                 the given VIOS.
        """
        npiv_map = self._build_map.get('npiv', {})
        return npiv_map.get(vios_w.uuid, {}).get(fabric, None)


class RebuildSlotMap(object):
    """Used to determine the slot topology when rebuilding a VM.

    A LPAR rebuild needs to configure the slots with the exact topology as
    their source.  This implementation requires additional details from the
    ...
    def __init__(self, slot_store, vios_wraps, pv_vscsi_vol_to_vio,
        ...
        self._npiv_build_out(npiv_fabric_map)

    def _pv_vscsi_build_slot_order(self):
        slots_order = {}
    ...
    def _pv_vscsi_build_out(self, vol_to_vio):
        ...
        slots_order = self._pv_vscsi_build_slot_order()
        ...
        for slot in slots_order:
            ...
            for udid in self._slot_store.topology[slot][IOCLASS.PV]:
                ...
                self._put_pv_vscsi_slot(vios_uuid_for_slot, udid, slot)
    ...
    def _npiv_build_out(self, fabric_slot_map):
        """Builds the build map for the NPIV fabrics."""
        for fabric in fabric_slot_map:
            self._npiv_fabric_build_out(fabric)
            ...
            self.vios_wraps.reverse()

    def _npiv_fabric_build_out(self, fabric):
        """Builds the rebuild map for a specific NPIV fabric."""
        slots = fabric.get('slots', [])
        phys_wwpns = fabric.get('phys_wwpns', [])

        # Get a copy of the VIOSes that we can work off of.
        fabric_vioses = copy.copy(self.vios_wraps)

        # See if we can get enough viable VIOSes for the slots.  One client
        # slot per VIOS.
        necessary_slots = slots
    ...
    def _put_vfc_slot(self, vios_w, fabric, slot):
        if 'npiv' not in self._build_map:
            self._build_map['npiv'] = {}
        if fabric not in self._build_map['npiv']:
            self._build_map['npiv'][vios_w.uuid] = {}
        self._build_map['npiv'][vios_w.uuid][fabric] = slot

    def _put_vea_slot(self, mac, slot):
        if 'vea' not in self._build_map:
            self._build_map['vea'] = {}
        self._build_map['vea'][pvm_util.sanitize_mac_for_api(mac)] = slot

    def _put_mgmt_vea_slot(self, mac, slot):
        if 'mgmt_vea' not in self._build_map:
            self._build_map['mgmt_vea'] = {}
        self._build_map['mgmt_vea'] = {
            'mac': pvm_util.sanitize_mac_for_api(mac), 'slot': slot}

    def _put_pv_vscsi_slot(self, vios_uuid, udid, slot):
        if 'pv_vscsi' not in self._build_map:
            self._build_map['pv_vscsi'] = {}
        if vios_uuid not in self._build_map['pv_vscsi']:
            self._build_map['pv_vscsi'][vios_uuid] = {}
        self._build_map['pv_vscsi'][vios_uuid][udid] = slot

    def _put_npiv_slot(self, vios_w, fabric, slot):
        if 'npiv' not in self._build_map:
            self._build_map['npiv'] = {}
        if fabric not in self._build_map['npiv']:
            self._build_map['npiv'][vios_w.uuid] = {}
