...
class LparMetricParser(object):
    ...
    def __init__(self, adapter, phyp_ltm, vios_ltms):
        """Initiatlizes the metrics.
        ...
        :param vios_ltms: A list of the LTMMetrics for the Virtual I/O Server
                          components.
        """
        self._phyp = phyp_mon.PhypInfo(adapter.read_by_href(phyp_ltm.link))
        self._vios = [vios_mon.ViosInfo(adapter.read_by_href(x.link))
                      for x in vios_ltms]

        self.vm_data = {}
        for lpar_sample in self._phyp.sample.lpars:
            lpar_metric = LparMetric(lpar_sample.uuid)
            self.vm_data[lpar_metric.uuid] = lpar_metric

            # Fill in the Processor data.
            lpar_metric.proc = LparCPU(lpar_sample.processor)
            ...
            if lpar_sample.network is None:
                ...
                self._saturate_network(lpar_metric, lpar_sample.network)
            ...
            if lpar_sample.storage is None:
                ...
                self._saturate_storage(lpar_metric, lpar_sample.storage)

    def _saturate_network(self, lpar_metric, net_phyp_sample):
    ...
    def _saturate_storage(self, lpar_metric, lpar_phyp_storage):
        ...
        for vadpt in lpar_phyp_storage.v_stor_adpts:
            vio_adpt = self._find_vio_vstor_adpt(vadpt)
        ...
        for phyp_vfc_adpt in lpar_phyp_storage.v_fc_adpts:
            vfc_adpt = self._find_vio_vfc_adpt(phyp_vfc_adpt)
    ...
    def _find_vio_vstor_adpt(self, phyp_vadpt):
        for vios_ltm in self._vios:
    ...
    def _find_vio_vfc_adpt(self, phyp_vfc_adpt):
        for vios_ltm in self._vios:
...
class LparMetric(object):
    ...
    def __init__(self, uuid):
        """Creates a LPAR Metric.  Data will be set by invoker.
         ...
         - proc - The LPAR's processor statistics.
