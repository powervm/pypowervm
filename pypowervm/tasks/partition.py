...
DOWN_VM_STATES = (bp.LPARState.NOT_ACTIVATED, bp.LPARState.ERROR,
...
def validate_vios_ready(adapter, max_wait_time=600):
    ...
    rmc_down_vioses = []
    sleep_step = 5
    wait_left = max_wait_time
    while True:
        try:
            vios_wraps = vios.VIOS.get(adapter)
            if not vios_wraps:
                # If there are truly no VIOSes (which should generally be
                # impossible if this code is running), break and fail.
                break
            rmc_down_vioses = [
                vwrap for vwrap in vios_wraps if vwrap.rmc_state not in
                VALID_RMC_STATES and vwrap.state not in DOWN_VM_STATES]
            if not rmc_down_vioses and get_active_vioses(
                    adapter, vios_wraps=vios_wraps):
                # At least one VIOS is up, and all active VIOSes have RMC.
                return
        ...
        wait_left -= sleep_step

    if len(rmc_down_vioses) > 0:
        LOG.warning(
