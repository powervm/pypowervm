...
class PanelJob(object):
    @classmethod
    def start(cls, part, opt=None, timeout=CONF.pypowervm_job_request_timeout,
              synchronous=True):
        ...
        except pvmex.JobRequestTimedOut as error:
            LOG.exception(error)
            raise error
        except pvmex.JobRequestFailed as error:
            LOG.exception(error)
            raise error

    @classmethod
    def _validate_args(cls, part, opt):
        if not part:
