...
class IBMiLoadSourceException(Exception):
    """Exceptions thrown from the update load source."""
    pass
...
def update_ibmi_settings(adapter, lpar_w, boot_type):
    ...
    TaggedIO of IBMi vm will be updated to idenfity the load source,
    alternative load source and console type. Keylock position will be set
    to one of the values of KeylockPos enumration. IPL Source will be set
    to one of the values of IPLSrc enumration.
    :param adapter: The pypowervm adapter.
    :param lpar_w: The lpar wrapper.
    :param boot_type: The boot connectivity type of the VM. It is a string
    value that represents one of the values in the BootStorageType enumeration.
    ...
    if boot_type == pvm_lpar.BootStorageType.VFC:
        msg = _LI("Setting Virtual Fibre Channel slot as "
        ...
        msg = _LI("Setting Virtual SCSI slot slot as load "
    ...
    if load_source is not None:
        ...
        msg = _LE("No load source found for VM %s") % lpar_w.name
        LOG.error(msg)
        raise IBMiLoadSourceException(msg)
