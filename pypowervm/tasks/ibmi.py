...
def update_load_src(adapter, lpar_w, boot_type):
    """Update load source of IBMi VMs on the instance.
    ...
    client_adapters = []
    if boot_type == 'vscsi':
        LOG.info(_LI('Starting loading vscsi slot number'))
        ...
        LOG.info(_LI('Starting loading npiv slot number'))
        vios_wraps = pvm_vios.VIOS.wrap(adapter.read(
            pvm_vios.VIOS.schema_type,
    ...
    if load_source is not None:
        ...
        LOG.error(_LE('Load source of %s not found.'), instance.name,
                  instance=instance)
