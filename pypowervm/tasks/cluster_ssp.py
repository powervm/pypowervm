...
def _upload_conflict(tier, luname, mkr_luname):
    ...
    lus = [lu for lu in lufeed if luname in lu.name]
...
def get_or_upload_image_lu(tier, lunm, vios_uuid, stream_func, b_size):
    ...
    :param lunm: The name of the image LU.  Note that the name may be shortened
                 to satisfy length restrictions.
    ...
    lunm = u.sanitize_file_name_for_api(
        lunm, max_len=c.MaxLen.FILENAME_DEFAULT - len(prefix))
    mkr_luname = prefix + lunm
    first = True
    while True:
        # Fetch/refresh the list of image LUs.
        lufeed = stor.LUEnt.search(tier.adapter, parent_type=stor.Tier,
                                   parent_uuid=tier.uuid, lu_type=IMGTYP)
        # Look for all LUs containing the right name.
        lus = [lu for lu in lufeed if lunm in lu.name]

        # Does the LU already exist in its final, uploaded form?  If so, then
        # only that LU will exist, with an exact name match.
        if len(lus) == 1 and lus[0].name == lunm:
            LOG.info(_('Using already-uploaded image LU %s.'), lunm)
            return lus[0]

        # Is there an upload in progress?
        if _upload_in_progress(lus, lunm, first):
            first = False
            time.sleep(sleep_s)
            continue
        ...
        tier, mkrlu = tsk_stg.crt_lu(tier, mkr_luname, 0.001, typ=IMGTYP)
        ...
        try:
            ...
            try:
                ...
                LOG.exception(_('Removing failed LU %s.'), lunm)
                new_lu.delete()
            return new_lu
        finally:
