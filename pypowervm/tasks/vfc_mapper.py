...
def find_maps(mapping_list, client_lpar_id,
              client_adpt=None, port_map=None):
    ...
    :param client_adpt: (Optional, Default=None) If set, will only include
                        ...
                        well.
    :param port_map: (Optional, Default=None) If set, will look for a
    ...
    match_func = lambda p_map: (
        ...
        p_map.backing_port.wwpn == port_map)
...
def add_vfc_mapping(vios, lpar_uuid, adapter, pfc_port_name):
    ...
    if find_maps(vios_w.vfc_mappings, client_lpar_id=lpar_uuid,
                 client_adpt="", port_map=pfc_wwpns):
    ...
    vios_w.vfc_mappings.append(vfc_map)
    LOG.info(_("Creating VFC mapping of %(pfc_port_name)s physical fc port"
               "from Virtual I/O Server %(vios_w_name)s to "
               "client LPAR %(lpar_uuid)s."),
             {'pfc_port_name': pfc_port_name, 'vios_w_name': vios_w.name,
              'lpar_uuid': lpar_uuid})
