...
def _split_ports_per_fabric(slot_grouping, fabric_data):
    resp = {}
    for fabric in fabric_data.keys():
        slots = [x for x in fabric_data['slots'] if x in slot_grouping]
        if not slots:
            continue

        resp[fabric] = {'slots': slots,
                        'p_port_wwpns': fabric_data['p_port_wwpns']}
    return resp
...
def build_migration_mappings(vios_wraps, fabric_data, slot_peers):
    ...
    :param slot_peers: An array of arrays.  Indicates all of the slots that
                       ...
                       (from fabric B).  And lastly one VIOS must only host
    ...
    slot_peers = sorted(slot_peers, len)
    ...
    resp = []
    for vios_w in vios_to_split_map.keys():
