...
def add_npiv_port_mappings(adapter, host_uuid, lpar_uuid, npiv_port_maps):
    ...
    new_maps = []
    for wrp_tsk_ret in ret.values():
        vios_w = wrp_tsk_ret.get('wrapper')
        for key, val in wrp_tsk_ret.iteritems():
            if not key.startswith('added_map_') or val is None:
                continue
            p_port_wwpn = _find_pfc_wwpn_by_name(vios_w, val.backing_port.name)
            mapping = (p_port_wwpn, ' '.join(val.client_adapter.wwpns))
