    ...
    if v_wwpns is not None:
        for vfc_map in vios_w.vfc_mappings:
            ...
            if vfc_map.backing_port:
                ...
                backing_port = bp.PhysFCPort.bld_ref(vios_w.adapter,
                                                     p_port.name)
                # Change the label.  The API requires it be Port,
                # even though it is a Physical FC Port.
                backing_port.element.tag = 'Port'
                vfc_map.backing_port = backing_port
                return vfc_map
