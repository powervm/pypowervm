...
def find_vios_for_vfc_wwpns(vios_wraps, vfc_wwpns):
    ...
    for vios_w in vios_wraps:
        for vfc_map in vios_w.vfc_mappings:
            ...
            # Make sure the vfc wwpns match
            full_match = True
            for v_wwpn in vfc_wwpns:
                if v_wwpn not in vfc_map.client_adapter.wwpns:
                    full_match = False
                    break

            # If the WWPNs didn't match, break to next mapping
            if not full_match:
                continue

            # Return the VIOS and port
...
def build_migration_mappings_for_fabric(vios_wraps, p_port_wwpns,
    ...
    for basic_map in basic_mappings:
        # We pop off the every slot.  Slot count should match the derive count.
