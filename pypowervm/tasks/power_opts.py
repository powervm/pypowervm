...
class _PowerOpts(object):
    ...
    def __str__(self):
        """String representation of this instance, for logging purposes."""
        parms = ', '.join(["%s=%s" % (key, val)
                           for key, val in six.iteritems(self._parm_map)])
    ...
    def _process_bool(self, key, value, include_if_false=False):
        """Process a boolean option.

        :param key: The JobParameterName.
        :param value: A bool (True/False) or string ('true', 'false',
                      case-insensitive).
        :param include_if_false: If False (the default), and 'value' evaluates
                                 to False, the parameter is not added to the
                                 parameter list.  The parameter is always added
                                 if it evaluates to True.
        """
        if key in self._parm_map:
            del self._parm_map[key]
        the_bool = str(value).lower() == 'true'
        if the_bool or include_if_false:
            self._parm_map[key] = str(the_bool).lower()
        return self
...
class PowerOffOpts(_PowerOpts):
    ...
    def os_normal(self):
        ...
        :return self: For chaining.
        """
        return self.operation(PowerOffOperation.OS).immediate(False)
    ...
    def os_immediate(self):
        ...
        :return self: For chaining.
        """
        return self.operation(PowerOffOperation.OS).immediate(True)
    ...
    def vsp_normal(self):
        ...
        :return self: For chaining.
        """
        return self.operation(PowerOffOperation.VSP).immediate(False)
    ...
    def vsp_hard(self):
        ...
        :return self: For chaining.
        """
        return self.operation(PowerOffOperation.VSP).immediate(True)
    ...
    def soft_detect(self, part, immed_if_os=None):
        ...
        if self.can_os_shutdown(part):
            ...
            if immed_if_os is not None:
                self.immediate(immed_if_os)
            else:
                # Default is normal for IBMi, immediate for non-IBMi.
                self.immediate(part.env != bp.LPARType.OS400)
        else:
