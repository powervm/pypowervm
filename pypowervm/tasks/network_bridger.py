...
class NetworkBridgerVNET(NetworkBridger):
    ...
    def _find_available_ld_grp(self, nb):
        ...
        if nb.load_balance and cur_lg is not None:
            # This flags that load balancing is enabled.  But if there is only
            # one available, then we would end up 'packing' all the VLANs on
            # to that.  Therefore, we need to make sure we have at least two
            # available at all times with this load balancing mode.
            fuzzy_min = _MAX_VLANS_PER_VEA / 2

            # Note the fuzzy min...if we have less than half the max VLANs, we
            # should just make a new vNet.  If we have more than that, we're
            # probably just filling up a pair that was created before.
            if avail_count == 1 and len(cur_lg.vnet_uri_list) <= fuzzy_min:
                return None
...
class NetworkBridgerTA(NetworkBridger):
    ...
    def _find_available_trunks(self, nb):
        ...
        if nb.load_balance and cur_min is not None:
            # Essentially, we need to make sure that there was at least two
            # available adapters.  Otherwise we're 'packing' them in too
            # tightly.
            fuzzy_min = _MAX_VLANS_PER_VEA / 2

            # Note the fuzzy min...if we have less than half the max VLANs, we
            # should just make a new Trunk.  If we have more than that, we're
            # probably just filling up a pair that was created before.
            if avail_count == 1 and len(cur_min.tagged_vlans) <= fuzzy_min:
