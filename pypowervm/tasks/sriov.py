...
def set_vnic_back_devs(vnic_w, pports, sriov_adaps=None, vioses=None,
    ...
    :raise NoActiveVios: If no active (including RMC) VIOSes can be found.
    ...
    vioses = tpar.get_active_vioses(adap, xag=[], vios_wraps=vioses, find_min=0)
    ...
    card_use = {pport.sriov_adap_id: 0 for pport in pport_wraps}
    sriov_adap_ids = [pp.sriov_adap_id for pp in card_use]
    vio_idx = 0
    while pport_wraps and len(vnic_w.back_devs) < backdev_goal:
        # Always alternate VIOSes
        vio = vioses[vio_idx]
        vio_idx = (vio_idx + 1) % len(vioses)
        # Sort the adapters in order of least-used (for this vNIC)
        sriov_adap_ids.sort(key=lambda said: card_use[said])
        # Find the next emptiest port on the least-used card.
        pp2use = [pport for sriov_adap_id in sriov_adap_ids for pport in
                  pport_wraps if pport.sriov_adap_id == sriov_adap_id][0]
        # Register a hit on the chosen port's card
        card_use[pp2use.sriov_adap_id] += 1
        # Create the backing device
        vnic_w.back_devs.append(card.VNICBackDev.bld(
            adap, vio.uuid, pp2use.sriov_adap_id, pp2use.port_id,
            capacity=capacity))
...
def _get_good_sriovs(adap, sriov_adaps=None):
    ...
    sriov_adaps = [sriov for sriov in sriov_adaps if
    ...
    LOG.debug('Found running/shared SR-IOV adapter(s): %s',
              str([sriov.phys_loc_code for sriov in sriov_adaps]))


def _get_good_pport_list(sriov_adaps, pports, capacity, min_returns):
    """Get a filtered list of SRIOV*PPort, ordered by allocated capacity.
    ...
    - The list is sorted in ascending order of allocated capacity (that is, the
      least-saturated ports come first).
    ...
    """
    # Include ports listed in pports.
    # Exclude ports with insufficient capacity.
    pport_wraps = []
    for sriov in sriov_adaps:
        for pport in sriov.phys_ports:
            ...
            if pport.loc_code not in pports:
                continue
            # Does it have space?
            min_cap = pport.min_granularity if capacity is None else capacity
            if pport.allocated_capacity + min_cap > 1.0:
                continue
    ...
    pport_wraps.sort(key=lambda p: p.allocated_capacity)
