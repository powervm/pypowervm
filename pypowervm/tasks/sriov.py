...
def _vnics_using_pport(pport, lpar2vnics):
    ...
    :param pport: pypowervm.wrappers.iocard.SRIOV*PPort wrapper containing the
                  ...
                  physical port.
    :param lpar2vnics: Dict of {LPAR: [VNIC, ...]} gleaned from _get_lpar_vnics
    ...
    """
    warnings = []
    for lpar, vnics in lpar2vnics:
        # TODO(IBM): if not lpar.can_lpm(): continue ?
        # Can't actually use can_lpm, because that guy checks things like RMC
        # (i.e. whether we're ready to LPM right *now*).  But if the LPAR is
        # not migratable - e.g. if mem_dlpar or proc_dlpar are off, or if any
        # storage is non-virtual - then we shouldn't let it block relabel even
        # if it's got a vNIC.
        for vnic in vnics:
            if any([backdev for backdev in vnic.back_devs if
                ...
                warnings += _(
                    "SR-IOV Physical Port at location %(loc_code)s is backing "
                    "a vNIC belonging to LPAR %(lpar_name)s (LPAR UUID: "
                    "%(lpar_uuid)s; vNIC UUID: %(vnic_uuid)s).") % {
        ...
        for sriovadap in sys_w.asio_config.sriov_adapters:
            for pport in sriovadap.phys_ports:
                ...
                if pport.label == label_index[pport.loc_code]:
                    continue
                # Now we have to check all the VNICs on all the LPARs.
                if lpar2vnics is None:
                    lpar2vnics = _get_lpar_vnics(sys_w.adapter)
