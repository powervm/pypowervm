...
def derive_base_npiv_map(vios_wraps, p_port_wwpns, v_port_count):
    ...
    derive method assumes knowledge of the Virtual Fibre Channel mappings
    beforehand.  This method will generate a similar map, but when sent to the
    add_npiv_port_mappings method, that method will allow the API to generate
    the WWPNs rather than pre-seeding them.
    ...
    return derive_base_npiv_map(vios_wraps, p_port_wwpns, v_port_markers)
...
def add_npiv_port_mappings(adapter, host_uuid, lpar_uuid, npiv_port_maps):
    ...
    This method supports WWPNs that are build either through the
    derive_npiv_map or the derive_base_npiv_map.
    ...
    port_map = []
    related_href = pvm_vios.VStorageMapping.crt_related_href(
        adapter, host_uuid, lpar_uuid)
    for vios in updated_vioses:
        for vfc_mapping in vios.vfc_mappings:
            if vfc_mapping.client_lpar_href == related_href:
                ...
                mapping = (p_wwpn, _fuse_vfc_ports(c_wwpns))
...
def _mapping_actions(adapter, host_uuid, npiv_port_maps, func):
    ...
    :return: List of VIOS wrappers that were updated based on the request.
    """
...
def _add_npiv_port_map(host_uuid, vm_uuid, vios_wraps, npiv_port_map):
    ...
    vios_w, p_port = find_vio_for_wwpn(vios_wraps, npiv_port_map[0])
    v_wwpns = None
    if npiv_port_map[1] != _FUSED_ANY_WWPN:
        v_wwpns = set([u.sanitize_wwpn_for_api(x)
                       for x in npiv_port_map[1].split()])
