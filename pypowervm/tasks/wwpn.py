...
def derive_npiv_map(vios_wraps, p_port_wwpns, v_port_wwpns):
    ...
    p_port_wwpns = map(u.sanitize_wwpn_for_api, p_port_wwpns)
...
def _find_map_port(potential_ports, mappings):
    ...
    # as possible.
    #
    # This dictionary will have a key of every ports WWPN, within that it will
...
def _fuse_vfc_ports(wwpn_list):
    ...
    l = map(u.sanitize_wwpn_for_api, wwpn_list)
    return map(' '.join, zip(l[::2], l[1::2]))
