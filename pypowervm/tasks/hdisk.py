...
def _process_hdisk_inv_result(result):
    ...
    if xml_resp is None:
        return None
    naa = None
    # Todo: VIOS output xml has an expat error due to which we need
    # remove the strip VIO tag and process the result.
    ...
    for pv in root.getiterator('PhysicalVolume_base'):
        if ('desType' in pv.attrib.keys() and pv.attrib['desType'] == "NAA"):
