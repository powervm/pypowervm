...
def discover_hdisk_scrub_retry(adapter, vios_uuid, itls, lpar_id,
                               vendor=LUAType.IBM):
    """Try discover_hdisk; on failure, scrub stale LPAR storage and retry.

    See discover_hdisk.  This method attempts that call and analyzes the
    ...
    retry discover_hdisk.  The retry is only attempted once; that result is
    returned regardless.
    ...
    """
    # First attempt
    status, devname, udid = discover_hdisk(adapter, vios_uuid, itls, vendor)
    ...
    if devname is None or status not in (LUAStatus.DEVICE_AVAILABLE,
        ...
        scrub_task.execute()
        status, devname, udid = discover_hdisk(adapter, vios_uuid, itls,
                                               vendor)
