...
class _VNCRepeaterServer(threading.Thread):
    ...
    def _check_http_connect(self, client_socket, vnc_path):
        ...
        :returns http_code: The HTTP Connection code used for the client
                            connection.
        """
        # Connect string is 'prefix' + HTTP code + 'suffix'
        prefix = "CONNECT %s HTTP/" % vnc_path
        suffix = "\r\n\r\n"

        # Read in the connect string. HTTP code is length 3. Ex, '1.1' or '2.0'
        value = client_socket.recv(len(prefix) + 3 + len(suffix))

        # See if there is an HTTP version...if not default to '1.1'
        http_code = '1.1'
        http_pos = value.find('HTTP/')
        if http_pos > 0:
            # The HTTP code start is AFTER the HTTP/
            http_start = http_pos + 5
            http_end = http_start + 3
            http_code = value[http_start:http_end]

        # Make sure the strings match
        matcher = prefix + http_code + suffix
        return matcher == value, http_code
