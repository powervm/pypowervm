...
def open_vnc_vterm(adapter, lpar_uuid, bind_ip='127.0.0.1'):
    ...
    if not tty:
        ...
        # TODO(thorst) remove sudo when mkvtermutil is updated.
    ...
    if not _has_vnc_running(tty, port, bind_ip):
...
def _check_for_tty(lpar_id):
    ...
    # There are some variations, so we key off the absolute.
...
def _has_vnc_running(tty, port, listen_ip):
