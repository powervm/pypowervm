...
class _VNCRepeaterServer(threading.Thread):
    ...
    def _check_http_connect(self, client_socket, vnc_path):
        ...
        pat = "^CONNECT\s+(\S+)\s+HTTP/(.*)\r\n\r\n$"
        res = re.match(pat, value)
        http_code = res.groups()[1] if res else '1.1'

        # Can build the full expected connect path, once you have the http_code
        expected_path = ("CONNECT %s HTTP/%s\r\n\r\n") % (vnc_path, http_code)
        return (value == expected_path), http_code
